...
samp_reads <- read_delim(demuxsum_fp) %>%
  ...
  merge(select(s_meta, sample_id, uniq_sample_name), by = "sample_id", all = TRUE) %>%
...
freyja <- freyja_fp %>%
  ...
  rename(sample_id = 1) %>%
  mutate(sample_id = gsub("_S[0-9]*|_variants.tsv", "", sample_id))
...
cecret_aggregate <- here("data", "processed_cecret", "cecret_results.csv") %>%
  read_csv() %>%
  select(-c(sample_id, samtools_meandepth_after_trimming, samtools_per_1X_coverage_after_trimming, fasta_line, freyja_summarized)) %>%
...
sample_type_color_select <- c(15, 19, 16, 13, 2, #colors for sequencing controls
                              ...
                              1, 4, 9, #colors for wastewater samples
...
freyja_grouping <- s %>%
  as.data.frame() %>%
  select(sample_id, ww_group, freyja_lineages, freyja_abundances) %>%
  group_by(sample_id) %>%
  ...
  mutate(lineages = gsub("\\([0-9]*\\)|-like", "", lineages),
         lineages = ifelse((is.na(lineages) | lineages == "Misc" | relative_abundances == "nan"), "Unresolved", lineages),
         relative_abundances = as.numeric(gsub("nan", "", relative_abundances)),
...
freyja_sample_type <- freyja_grouping %>%
  ...
  droplevels() %>%
  complete(nesting(sample_id, ww_group), nesting(combined_lineage, main_lineage), fill = list(relative_abundances = 0)) %>%
  group_by(sample_id, ww_group, combined_lineage, main_lineage) %>%
  summarize(relative_abundances = sum(relative_abundances)) %>%
  ungroup() %>%
  ...
  group_by(ww_group, combined_lineage, main_lineage) %>%
