...
sequencing_metadata <- d %>%
  ...
  mutate(Target = case_when(concentration == "avg_conc" ~ paste0(Target, "_gc/uL"),
         ...
         `bs-ww_surv_system_sample_id` = paste("WW",
                             ...
                             format(run_date, format = "%y%m%d"),
                             suffix, sep = "-")) %>%
  select(uniq_sample_name, sample_group = "Type", sample_collect_date, sample_received_date = "Sample_date", Target, values, ddpcr_replicates, `bs-ww_surv_system_sample_id`) %>%
