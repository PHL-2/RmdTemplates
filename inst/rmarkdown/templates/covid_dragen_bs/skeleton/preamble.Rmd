...
if(is.na(prj_dir)) {
  stop (simpleError("Generate the metadata sheet and config files first using the generate_barcodes_IDT.R script"))
}

source(here(prj_dir, "config", "COVIDSeq_config.R"))
...
mapping_file_fp <- here(prj_dir, "metadata", list.files(here(prj_dir, "metadata"), pattern = ".csv"))
...
#b2fq_c_fp <- here(prj_dir, "Data", "bcl2fastq", "commands.tsv")
#b2fq_v_fp <- here(prj_dir, "Data", "bcl2fastq", "bcl2fastqVersion.stderr.txt")

### demultiplexing results from BaseSpace
gen_fq_fp <- here(prj_dir, "Data", "demux_reads", "CompletedJobInfo.xml")
adaptertrimming_fp <- here(prj_dir, "Data", "demux_reads", "AdapterTrimming.txt")
#demuxsum_fp <- here(prj_dir, "Data", "demux_reads", "DemuxSummaryF1L1.txt")
demuxsum_fp <- here(prj_dir, "Data", "demux_reads", "DemultiplexSummaryF1L1.txt")
fastqsum_fp <- here(prj_dir, "Data", "demux_reads", "FastqSummaryF1L1.txt")
SampleSheet_fp <- here(prj_dir, "Data", "demux_reads", "SampleSheetused.csv")

### kmer fp
kmer_fp <- here(prj_dir, "Data", "kmer", "Data export.csv")

### NextClade results fp
nextclade_fp = here(prj_dir, "Data", "NextClade", "Data export.csv")

### Pangolin
pangolin_fp <- here(prj_dir, "Data", "Pangolin", "Data export.csv")
