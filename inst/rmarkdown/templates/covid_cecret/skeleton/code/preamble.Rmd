...
primer_select <- 1 # set variable as 1 for V3 or 2 for V5.3.2
...
amplicon_fp <- here("data", "processed_cecret", "aci", "amplicon_depth.csv")

amplicon_depth <- read_csv(amplicon_fp) %>%
  rename(sample_id = "bam") %>%
  mutate(sample_id = gsub(".primertrim.sorted.bam", "", sample_id)) %>%
  rename_with(function(x) paste0("amp_tile_", x, "_artic_", artic_primer_scheme), -sample_id)
...
list(samp_reads, cov_stats, coverage, next_clade, freyja, amplicon_depth, samtools_stat) %>%
...
```{r eval = TRUE}
  ...
  variant_color <- data.frame(colors = c(calc_pal()(12)[c(6, 2:5, 1, 8:12, 7)], few_pal(palette = "Light")(8))) %>%
  ...
  for(var in unique(variant_color$lineage)) {
    ...
    variant_gradient <- colorRampPalette(c(unique(subcolors$colors), adjustcolor(unique(subcolors$colors), alpha.f = 0.1)), alpha = TRUE)(nrow(subcolors)) 
  ...
  ann_geom_values[["who_label"]] <- var_colorgrad %>%
...
for(who_label in c("Reference", sort(unique(s$nextclade_clade_who)[unique(s$nextclade_clade_who) != "Reference"]))) {
  ...
  variant_list <- s %>%
    filter(nextclade_clade_who == "Omicron") %>%
    #is_designated means that the sample was assigned to a reference strain exactly using PANGO algorithm (decision tree) rather than PUSHER
    #the pangolin_note section here for PUSHER will be 'Assigned from designation hash.' so its useless
  ...
  for(combined_variant in combined_list) {
    ...
    if(grepl("[0-9]$", combined_variant)) {
      ...
      #make the sublineage regex
      combined_variant_gsub <- gsub("\\.", "\\\\.", combined_variant)
      exact_lineage_regex <- paste0("^", combined_variant_gsub, "$")
      sublineage_regex <- paste0("^", combined_variant_gsub, "\\.")
      final_sublineage_regex <- paste0(exact_lineage_regex, "|", sublineage_regex)
      
      sublineage <- variant_list[grepl(final_sublineage_regex, variant_list)]
    } else{
      sublineage <- combined_variant
    }
    
    variant_color <- c(calc_pal()(12)[c(6, 2:5, 1, 8:12, 7)], few_pal(palette = "Light")(8))[i]
...
for(col_num in col2show) {
  ...
  filtered_samp_name <- s %>%
    ...
    mutate(who_label = case_when(is.na(Nextclade_pango) & !isControl ~ "no results",
                                 is.na(Nextclade_pango) & isControl ~ "control",
                                 TRUE ~ paste0(nextclade_clade_who, " - ", gsub("^([A-Za-z]*\\.[0-9]*).*", "\\1", Nextclade_pango)))) %>%
    mutate(who_label = factor(who_label, levels = c(names(ann_geom_values$who_label), "no results", "control"))) %>%
    filter(sample_id != "Undetermined") %>%
  ...
  slide_window_coverage %>%
    ...
    droplevels() %>%
    {
    ggplot(., aes(x = position, y = slide_window_100, group = sample_id, fill = who_label)) +
      ...
      scale_fill_manual(values = c(ann_geom_values$who_label, setNames(c("#808080", "#000000"), c("no results", "control")))[match(levels(.$who_label),
                                                                                                  c(names(ann_geom_values$who_label), "no results", "control"))]) +
      scale_x_continuous(expand = c(0,0), limits = c(0, 30000)) + 
      theme_bw() +
      theme(
        strip.background = element_blank(),
