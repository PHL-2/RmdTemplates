...
k2 %>%
  ...
  sample_type_legend +
    geom_text(aes(label = plate_coord), size = convert_sample_size_2_font_size(nrow(s)))
...
snp_dist_reorder <- snpdists %>%
  ...
  merge(select(s, sample_id, sample_type, plate_coord), by.x = "query", by.y = "sample_id", all.x = TRUE) %>%
  rename(query_sample_type = "sample_type",
         query_plate_coord = "plate_coord") %>%
  mutate(query_plate_coord = gsub(".*_", "", query_plate_coord)) %>%
...
snp_dists_y_legend <- snp_dist_reorder %>%
  ...
  ggplot(., aes(x = 1, y = subject, fill = subject_sample_type)) +
    geom_tile(color = "black") +
    geom_text(aes(label = subject_plate_coord), size = convert_sample_size_2_font_size(nrow(s_toPlot))) +
    coord_fixed() +
...
snp_dists_x_legend <- snp_dist_reorder %>%
  select(query, query_sample_type, query_plate_coord) %>%
  unique() %>%
  {
  ggplot(., aes(y = 1, x = query, fill = query_sample_type)) +
    geom_tile(color = "black") +
    geom_text(aes(label = query_plate_coord), size = convert_sample_size_2_font_size(nrow(s_toPlot))) +
    coord_fixed() +
...
read_type %>%
  ...
  read_type_legend +
    geom_text(aes(label = plate_coord), size = convert_sample_size_2_font_size(nrow(s), max_font = 1.5))
