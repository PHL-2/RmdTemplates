...
freyja_average %>%
  group_by(sample_group, isMerged, CDC_lineages) %>%
  ...
  ggplot(., aes(x = isMerged, y = avg_rel_abund, fill = CDC_lineages)) +
    ...
    scale_fill_manual(values = 
                        c(colorRampPalette(brewer.pal(11, "Spectral"))(length(unique(.$CDC_lineages))-2),
                          "#808080", "#000000")) +
...
Relative lineage abundances within each sample group. Standard deviation is calculated by adding the total variance of a lineage across each sample within the same sample group and getting the square root \newline
...
freyja_table_delim <- freyja_average %>%
  group_by(sample_group, isMerged, CDC_lineages, main_lineage) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(CDC_lineages != "Other lineages",
         avg_rel_abund >= 0.01) %>%
  arrange(CDC_lineages) %>%
  select(CDC_lineages, main_lineage) %>%
  unique() %>%
...
freyja_average %>%
  ...
  mutate(print_rel_abund = ifelse(!is.na(total_sd),
                                  ...
                                  paste0(round(avg_rel_abund*100, 2), "%")),
         sample_group = paste(sample_group, isMerged, sep = "\n")) %>%
  select(-c(total_sd, avg_rel_abund, isMerged)) %>%
  unique() %>%
  pivot_wider(names_from = "sample_group", values_from = "print_rel_abund", values_fill = "") %>%
  filter(!is.na(CDC_lineages)) %>%
  arrange(CDC_lineages) %>%
  rename(Lineage = "CDC_lineages") %>%
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      ...
      row_spec(freyja_table_delim, hline_after = T)
    else
      pander(.)
  }
...
freyja_table_delim <- freyja_average %>%
  group_by(sample_group, isMerged, CDC_lineages, lineages, main_lineage) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(CDC_lineages == "Other lineages",
...
freyja_average %>%
  group_by(sample_group, isMerged, CDC_lineages, lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(CDC_lineages == "Other lineages",
  ...
  mutate(print_rel_abund = ifelse(!is.na(total_sd),
                                  ...
                                  paste0(round(avg_rel_abund*100, 2), "%")),
         sample_group = paste(sample_group, isMerged, sep = "\n")) %>%
  select(-c(total_sd, avg_rel_abund, isMerged, CDC_lineages)) %>%
  unique() %>%
  pivot_wider(names_from = "sample_group", values_from = "print_rel_abund", values_fill = "") %>%
  filter(!is.na(lineages)) %>%
  arrange(lineages) %>%
  rename(Lineage = "lineages") %>%
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      select_all(~gsub(" ", "\n", .)) %>%
      select_all(~gsub("_", "\\\\_", .)) %>%
