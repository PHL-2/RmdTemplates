...
dcipher_meta_fp <- here("upload", "dcipher", paste(project_name, "PHL2_dcipher_upload.tsv", sep = "_"))
...
```{r export data to epi, eval = FALSE}
...
# Export the data for dcipher upload to sra, genbank, and gisaid
dcipher <- s_toPlot %>%
  filter4report() %>%
  #submit only nasal swabs for now. WW data needs additional metadata
  filter(sample_type == "Wastewater") %>%
  mutate(sample_collect_date = case_when(!(is.na(sample_collect_date) | as.character(sample_collect_date) == "") ~ as.character(sample_collect_date),
                                     TRUE ~ NA)) %>%
  ...
  mutate(sample_collect_year = format(sample_collect_date, format = "%Y")) %>%
  ...
  select(sample_id, genbank_name, sra_name, gisaid_name, gisaid_accession, sample_collect_date, location, gisaid_location, zip, type, passage, organism,
...
run_check_all_chunk <- any(grepl("Nasal swab", color_group_lvls))
...
freyja_grouping <- s %>%
  as.data.frame() %>%
  select(sample_id, sample_group, ww_group, freyja_lineages, freyja_abundances, pct_genome_coverage_over_10x) %>%
  group_by(sample_id) %>%
...
freyja_matrix <- freyja_grouping %>%
  complete(nesting(sample_id, sample_group), combined_lineage, fill = list(relative_abundances = 0)) %>%
...
#add filter to remove low abundance lineages (<1%)
freyja_sample_type %>%
  ...
  pivot_wider(names_from = "sample_group", values_from = "print_rel_abund", values_fill = "") %>%
