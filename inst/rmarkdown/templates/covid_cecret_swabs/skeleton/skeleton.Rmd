...
```{r filter out samples, message = TRUE, eval = FALSE}

source("code/1_remove_low_RLU.R")

```

```{r generate SampleSheet and metadata, message = TRUE, eval = FALSE}

source("code/2_generate_barcodes_IDT.R")

```

```{r child = 'code/preamble.Rmd', message = NA, eval = load_preamble}
```
