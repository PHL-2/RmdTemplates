...
### packages install
### ================
install.packages(c("knitr", "here", "tidyverse", "rvest", "pander", "kableExtra", "RColorBrewer", "readxl", "patchwork", "zoo", "ggthemes", "stringdist", "openxlsx"))
...
```{r filter out samples, eval = FALSE, message = TRUE}
...
s %>%
  filter(isControl) %>%
  mutate(sample_type_name = ifelse(grepl("Environmental control", sample_type), paste0(sample_type, ":\n", environmental_site),
                                   paste0(sample_type, ": ", environmental_site))) %>%
  mutate(sample_type_name = ifelse(is.na(environmental_site), gsub(": NA$", "", sample_type_name), sample_type_name)) %>%
  order_on_other_col(sample_type_name, read_counts) %>%
  droplevels() %>%
  {
  ggplot(., aes(x=read_counts, y=sample_type_name, fill = sample_type)) +
    geom_col() +
...
empty_plate %>%
  ...
  mutate(plate_col = as.numeric(plate_col)) %>%
  droplevels() %>%
  mutate(nc_qc_status = factor(nc_qc_status, levels = c("bad", "mediocre", "good"))) %>%
  {
  ggplot(., aes(x=plate_col, y=fct_rev(plate_row), fill = nc_qc_status, label = sample_type)) +
    ...
    scale_fill_manual(values = brewer.pal(6, "Set3")[c(4, 6, 1)]) +
