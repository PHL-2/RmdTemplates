...
s %>%
  ggplot(aes(y=read_counts, x=sample_type, fill = sample_type)) +
    ...
    labs(
      title = paste0("Total number of reads: ", round(sum(s$read_counts)/1000000, digits = 2), "M"),
      y="Read counts",
      x="",
      fill = "Sample Type",
      size = "Sample Type"
...
s %>%
  ...
  droplevels() %>%
  select(sample_type, median_coverage, pct_genome_coverage_over_10x) %>%
  {
  ggplot(., aes(x = median_coverage, y = pct_genome_coverage_over_10x, fill = sample_type)) +
...
s %>%
  select(sample_type, median_coverage, pct_genome_coverage_over_10x, nextclade_qc_overallstatus) %>%
  filter(!is.na(median_coverage)) %>%
  ...
  { if(use_kable) 
    ...
    mutate(sample = cell_spec(sample, "latex",
                               color = case_when((grepl(paste0(rerun_samples, collapse = "|"), sample_id) & length(rerun_samples) > 0) ~ "yellow",
                                                 ...
                                                 ((grepl("bad", nextclade_qc_overallstatus)) | (grepl("mediocre", nextclade_qc_overallstatus) & pct_genome_coverage_over_10x < .6) | (grepl("fail", pangolin_qc_status))) ~ "orange",
                                                 TRUE ~ "black"))) %>%
      select(sample,
             `sample type`,
             `read counts`,
