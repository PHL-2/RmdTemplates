...
freyja_average %>%
  ...
  ungroup() %>%
  filter(avg_rel_abund >= 0.01) %>%
  left_join(select(s_meta, sample_group, sample_collect_date, `N2_gc/L`),
            by = c("sample_group", "sample_collect_date")) %>%
  ...
  select(`Sample group` = sample_group_label, `Mean coverage` = "mean_cov",
         Lineage = "CDC_lineages", `Relative abundance` = "relative_abundance", `N2_gc/L`, good_cov) %>%
  unique() %>%
  { if(use_kable)
...
freyja_average %>%
  ...
  select(`Sample group` = sample_group_label, `Mean coverage` = "mean_cov",
         Lineage = "lineages", `Relative abundance` = "relative_abundance", `N2_gc/L`) %>%
  unique() %>%
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      select_all(~gsub(" ", "\n", .)) %>%
