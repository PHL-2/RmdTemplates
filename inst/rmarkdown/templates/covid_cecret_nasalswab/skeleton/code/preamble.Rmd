...
if(max(top_unkwn_indices$percent_of_all_reads) > 0.005) {
  message("\n\nWarning:\nTop unknown barcode sequence makes up more than 0.5% of all reads. There may be a misassigned barcode!")
  Sys.sleep(10)
}
...
ec2_upload_fp <- paste(seqsender_upload_tmp_fp, "NS", project_name, "processed_cecret", "filter", sep = "/")
...
seqsender <- s_toPlot %>%
  ...
  mutate(orig_lab_address = case_when(orig_lab == "Philadelphia Department of Public Health" ~ "1930 South Broad Street Suite 37, Philadelphia, PA 19145 USA",
                                      ...
                                      TRUE ~ "Unknown")) %>%
  mutate(sra_file_path_1 = paste0(ec2_upload_fp, "/", sample_id, "_S", sample_number, "_filtered_R1.fastq.gz"),
         sra_file_path_2 = paste0(ec2_upload_fp, "/", sample_id, "_S", sample_number, "_filtered_R2.fastq.gz")) %>%
