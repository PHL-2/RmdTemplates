...
ann_geom_values[["linetype"]] <- setNames(c("solid", "twodash", "dotted"), c("good", "mediocre", "bad"))
...
col2show <- s %>%
  filter(!isControl & sample_id != "Undetermined") %>%
  select(plate_col) %>%
  unique() %>%
  pull()
...
for(col_num in col2show) {
  ...
  filtered_samp_name <- s %>%
    ...
    droplevels() %>%
    select(sample_id, lineage, nc_qc_status)

  slide_window_coverage %>%
    filter(sample_id %in% filtered_samp_name$sample_id) %>%
    merge(filtered_samp_name, by = "sample_id", all = TRUE) %>%
    {
    ggplot(., aes(x = position, y = slide_window_100, color = lineage, group = sample_id, linetype = nc_qc_status)) +
      geom_line() +
      ...
      labs(x="", y="Sliding window average (X)", color = "Variant", linetype = "Nextclade Quality") +
    annotation_legend + plot_layout(ncol = 1, heights = c(50, 1))
    } %>%
