...
### Read length after quality score, Illumina adapter, and ARTIC primer trimming
...
```{r k2 heatmap, fig.width=11, fig.height=9}
...
```{r k2 heatmap phyla, fig.width=9, fig.height=11}
...
k2 %>%
  ...
  mutate(k2_phyla = fct_lump(k2_phyla, n = 20, w = k2_props, other_level = "Other phyla")) %>%
  group_by(sample_id, k2_phyla) %>%
  summarise(k2_props = sum(k2_props)) %>%
  ungroup() %>%
  complete(sample_id, k2_phyla, fill = list(k2_props = 0)) %>%
  mutate(k2_phyla = sub(" ", "\n", k2_phyla)) %>%
  group_by(k2_phyla) %>%
  mutate(mean_props = mean(k2_props)) %>%
  ungroup() %>%
  ...
  order_on_other_col(sample_id, sample_type, decreasing = FALSE) %>%
  {
    ...
    labs(x = "", y = "", fill = "Relative\nProportion", title = "Top 20 most abundant phyla across all samples")
    } /
  sample_type_legend
...
snp_dist_reorder <- snpdists %>%
  ...
  merge(select(s, sample_id, sample_type), by.x = "subject", by.y = "sample_id", all.x = TRUE) %>%
  rename(subject_sample_type = "sample_type") %>%
  ...
  mutate(subject_sample_type = ifelse(is.na(subject_sample_type), "Reference", as.character(subject_sample_type)),
         subject_sample_type = factor(subject_sample_type, levels = c("Reference", levels(s$sample_type))),
         query_sample_type = ifelse(is.na(query_sample_type), "Reference", as.character(query_sample_type)),
         query_sample_type = factor(query_sample_type, levels = c("Reference", levels(s$sample_type)))) %>%
  order_on_other_col(subject, subject_sample_type) %>%
  mutate(query = factor(query, levels = rev(levels(subject))))

snp_dists_y_legend <- snp_dist_reorder %>%
  select(subject, subject_sample_type) %>%
  unique() %>%
  {
...
read_type <- s %>%
  select(sample_id, sample_type, read_counts, samtools_raw_total_sequences, samtools_reads_mapped, samtools_reads_unmapped) %>%
  mutate(filtered = read_counts - samtools_raw_total_sequences) %>%
  rename(mapped = "samtools_reads_mapped",
         unmapped = "samtools_reads_unmapped") %>%
  select(sample_id, sample_type, read_counts, filtered, mapped, unmapped) %>%
  pivot_longer(cols = c("filtered", "unmapped", "mapped"), names_to = "read_type", values_to = "counts") %>%
  mutate(read_type = factor(read_type, levels = c("filtered", "unmapped", "mapped"))) %>%
...
read_type %>%
  ...
  ggplot(., aes(x = sample_id, y = counts, fill = read_type)) +
    geom_col() +
    scale_fill_manual(values = brewer.pal(11, "Spectral")[c(6, 1, 10)]) +
    ...
    labs(x = "", y = "Total read counts", fill = "Variants")
  } /
  read_type_legend
...
s_toPlot %>%
  select(sample_id, Nextclade_pango, sample_type) %>%
  group_by(sample_type) %>%
  mutate(fraction = 1/n()) %>%
  ungroup() %>%
...
### Figure showing conflicting variant assignments from Nextclade and pangolin
...
s_toPlot %>%
  ...
  mutate(uniq = case_when(pn & grepl("Pangolin|NextClade", software) ~ "*",
                          TRUE ~ "")) %>%
  mutate(software = factor(software, levels = c("NextClade", "Pangolin"))) %>%
  mutate(variants = factor(variants, levels = names(ann_geom_values$variants))) %>%
  mutate(nextclade_qc_overallstatus = factor(nextclade_qc_overallstatus, levels = c("good", "mediocre", "bad"))) %>%
  mutate(sample_id = factor(sample_id, levels = unique(sample_id[order(variants, decreasing = TRUE)]))) %>%
  droplevels() %>%
