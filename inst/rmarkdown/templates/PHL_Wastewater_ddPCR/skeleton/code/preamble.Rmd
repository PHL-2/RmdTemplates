...
```{r finding most recent data, eval = useFolderCopy}
...
```{r saving data to rstudio metadata folder (for github upload), eval = useFolderCopy}
...
d_var <- d %>%
  ...
  summarise(avg_pos_droplets = mean(Positives),
            ...
            gesd = ifelse(Spiked == "Spiked", PMCMRplus::gesdTest(`Conc(copies/uL)`, 2)$p.value[2], NA)
  ) %>%
  ungroup() %>%
  unique() %>%
