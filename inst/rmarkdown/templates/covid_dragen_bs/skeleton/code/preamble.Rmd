...
seqsender_fp <- "C:/Users/vincent.tu/OneDrive - City of Philadelphia/git_clones/seqsender"
...
```{r upload data to basespace and run the app, eval = dl_bs_data}
...
write(paste0(c("project_id", project_id), collapse = ","), here("data", paste0(sequencing_date, "_bs_metadata.csv")))
...
write(paste0(c("dragen_app_version", dragen_app_version), collapse = ","), here("data", paste0(sequencing_date, "_bs_metadata.csv")), append = TRUE)
...
app_session_id <- cli_submit(sh_exe_fp, bs_cli_launch_fp, c(shQuote(app2run, type = "sh"),
                                                              ...
                                                              project_id,
                                                              artic_primer_scheme))
write(paste0(c("app_session_id", app_session_id), collapse = ","), here("data", paste0(sequencing_date, "_bs_metadata.csv")), append = TRUE)

### 5 - wait until the app finished running
bs_cli_await_fp <- file.path(dirname(here()), "aux_files", "bs_cli_commands", "bs_cli_5_await.sh")
awaiting <- cli_submit(sh_exe_fp, bs_cli_await_fp, app_session_id)

### 6 - download data
bs_cli_download_fp <- file.path(dirname(here()), "aux_files", "bs_cli_commands", "bs_cli_6_download.sh")
downloading <- cli_submit(sh_exe_fp, bs_cli_download_fp, c(project_id,
...
app_run_date <- cli_submit(sh_exe_fp, bs_cli_get_date_fp, app_session_id) %>%
  as.Date() %>%
  as.character()
write(paste0(c("app_run_date", app_run_date), collapse = ","), here("data", paste0(sequencing_date, "_bs_metadata.csv")), append = TRUE)
...
slide_window_coverage <- coverage %>%
  group_by(sample_id) %>%
  mutate(slide_window_100 = rollmean(map_per_position, k=100, fill = NA, align = 'right')) %>%
  ungroup()
...
next_clade <- nc_files_fps %>%
  data_frame(FileName = .) %>%
  group_by(FileName) %>%
  do(read_delim(.$FileName, col_names = TRUE, col_types = cols(missing = col_character(), clade_nextstrain = col_character()))) %>%
  ungroup() %>%
  select(-c(FileName)) %>%
...
bs_meta <- read_csv(here("data", paste0(sequencing_date, "_bs_metadata.csv")), col_names = FALSE) %>%
  t() %>%
  `colnames<-`(.[1, ]) %>%
  as.data.frame() %>%
  slice(-1)
...
slide_window_coverage %>%
  saveRDS(here("data", paste0(sequencing_date, "_sliding_window.rds")))
...
slide_window_coverage <- readRDS(here("data", paste0(sequencing_date, "_sliding_window.rds")))
...
sample_type_fct <- c("Unassigned reads", "Water control", "Reagent control", "Mock DNA positive control")
...
control_gzip_fps <- gzip_fps[grepl(gsub("20221223", "20221222", paste0(s[s$isControl, "sample_id"], collapse = "|")), gzip_fps)]
...
lvls <- c("Unassigned reads", "Water control", "Reagent control", "Mock DNA positive control",
...
ann_geom_values[["alpha"]] <- setNames(c(1, 1, 1, 1, rep(0.2, length(lvls)-4)), lvls)
...
#loop over the VOCs
for(who_label in c("Reference", sort(unique(s$scorpio_call)[unique(s$scorpio_call) != "Reference"]))) {
  
  variant_list <- s %>%
    filter(scorpio_call == who_label) %>%
    ...
    select(lineage, note, Nextclade_pango, scorpio_lineage) %>%
    ...
    pivot_longer(cols = c("lineage", "pusher_variant1", "pusher_variant2", "Nextclade_pango", "scorpio_lineage"), names_to = "variant_num", values_to = "variants") %>%
