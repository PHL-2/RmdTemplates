...
`r samp_sh_header$Workflow` version `r demux_software_v["bclconvert"]` was used to generate and demultiplex sample FASTQ files from the sequencing run BCL files (raw data files that contain sequence information and quality scores). This program was initiated with the nf-core/demultiplex pipeline version `r demux_software_v["nf-core-demultuplex"]`, using Nextflow version `r demux_software_v["Nextflow"]`
...
Demultiplexed FASTQ files were processed with FALCO version `r demux_software_v["falco"]` to obtain FASTQC scores of the reads

```{r fastqc, fig.width=11, fig.height=8}

knitr::include_graphics(here("data", paste0(sequencing_date, "_fastqc.png")))
...
### Unassigned index Hamming distances to sample barcodes
...
top_unkwn_indices %>%
  ...
  ggplot(., aes(x=min_dist)) +
    ...
    scale_x_continuous(limits = c(0, max(.$min_dist) + 1),
                       breaks = scales::breaks_pretty(max(.$min_dist))) +
    labs(
      x="Hamming distance",
      y="Counts"
    )
...
amplicon %>%
  ggplot(aes(x = depth, y = fct_rev(amp_pos), alpha = 0.1)) +
    geom_boxplot(outlier.alpha = 0) +
    theme_bw() +
    ...
    labs(x="Depth", y="Ampicon position")
...
```{r freyja results, fig.height=9, fig.width=7}

s_toPlot %>%
  select(sample_id, freyja_lineages, freyja_abundances) %>%
  group_by(sample_id) %>%
  ...
  #mutate(sum_test = sum(relative_abundances)) %>%
  ungroup() %>%
...
s_toPlot %>%
  ...
  ungroup() %>%
  select(sample_type, PHL_sample_received_date, lineage, clade_who, all_samples_n) %>%
