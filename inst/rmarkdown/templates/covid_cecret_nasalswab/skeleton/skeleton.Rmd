...
\textcolor{red}{DISCLAIMER}: The identification methods used and the data summarized are for public health surveillance or investigational purposes only and must \textcolor{red}{NOT} be communicated to the patient, their care provider, or placed in the patient’s medical record. These results should \textcolor{red}{NOT} be used for diagnosis, treatment, or assessment of individual patient health or management.
...
dir.create(file.path(copy_to, project_name, "metadata"), recursive = TRUE, showWarnings = FALSE)

if(!file.copy(PHI_file_fp, file.path(copy_to, project_name, "metadata"), overwrite = TRUE)) {
  stop(paste("Could not copy file to", file.path(copy_to, project_name, "metadata"),
             "\nMaybe the shared drive is not connected"))
}
...
#run the {settings for processing the sequencing run} chunk above first
...
options("readr.show_col_types" = FALSE)
source("code/5_upload_data_2_public_repo.R")
