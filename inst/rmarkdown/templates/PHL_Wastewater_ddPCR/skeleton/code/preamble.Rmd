...
if(sum(grepl(pattern = "Sample description", x = colnames(d_pre)))>0) {
  ...
  d_pre <- d_pre %>%
    ...
    select(Well, Sample, Spiked, Sample_date, Extra, Target, secondary, value, Droplets) %>%
    pivot_wider(
      id_cols = c("Well", "Sample", "Spiked", "Sample_date", "Extra", "Target", "Droplets"),
...
if (variance_test) {
  ...
  all_var_check <- any(wellsToRemove %in% all_var_check)
