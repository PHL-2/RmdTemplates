...
### additional metadata
extra_file_fp <- here("metadata", "extra_metadata", list.files(here("metadata", "extra_metadata"), pattern = ".tsv|.csv"))
has_extra <- length(extra_file_fp) > 0
...
if(!all(c(samp_sh_check$I7_ID, samp_sh_check$I7_ID))) {
  stop(simpleError("One of the barcode ID's don't match! Check samp_sh_check variable"))
}
...
s <- s_meta %>%
  ...
  mutate(sample_type = ifelse(sample_id == "Undetermined", "Unassigned reads", sample_type)) %>%
...
```{r add more metadata, eval = has_extra}

extra_data <- read_delim(extra_file_fp)

s <- merge(s, extra_data, by = "sample_name", all = TRUE) %>%
  #remove samples without a UDI position but keep Undetermined
  filter(!is.na(UDI_Index_ID)|grepl("Undetermined", sample_id))
...
who_label_lab <- data.frame(x = 1, y = 1, fill_col = names(ann_colors[["who_label"]])) %>%
