...
s_toPlot <- s %>%
  ...
  filter(!(grepl("^B$|^B\\.|^A$|^A\\.", Nextclade_pango) & !isControl)) %>%
...
#indicator for samples to rerun
rerun_samples <- character(0)
...
fields4lims <- c("sample_id",
                 ...
                 "index2",
                 "prj_descrip",
                 "sequencing_request_date" = "PHL_sample_received_date",
                 "sequencing_date",
                 "instrument_type",
                 "read_length",
                 "index_length",
                 ...
                 "R2_Q30",
                 "aligned_reads",
                 "pct_genome_coverage_over_30x",
                 "pct_genome_coverage_over_1x",
                 "median_coverage",
                 "mean_coverage",
                 "k2_sc2_kmers",
                 "k2_sc2_props",
                 "nextclade_qc_overallstatus",
                  
                  #epi
                 "nextclade_pango_lineage" = "Nextclade_pango",
                 "who_label" = "nextclade_clade_who")
...
select_software <- read_csv(software_version_fp) %>%
  filter(grepl(paste("^Nextflow",
                     ...
                     "ivar", 
                     "nextclade",
                     "nextclade-dataset$", sep = "$|^"), software)) %>%
...
```{r export data to upload to NCBI, eval = (nrow(s_toPlot) > 0)}
...
seqsender <- s_toPlot %>%
  ...
  mutate(collection_date = case_when(!(is.na(sample_collection_date) |
         ...
         `gs-covv_sex` = ifelse(is.na(gender)|`gs-covv_orig_lab` == "Temple University", "Unknown", gender),
         ...
         `gs-covv_orig_lab_addr` = case_when(`gs-covv_orig_lab` == "Philadelphia Department of Public Health" ~ "1101 Market Street, Floor 13, Philadelphia, PA 19107, USA",
         ...
         `bs5-host_sex` = ifelse(`gs-covv_sex` == "Unknown", "Not provided", `gs-covv_sex`),
...
```{r reported numbers, eval = (nrow(s_toPlot) > 0)}
...
if(have_phi) {
  ...
  epi_2_upload_fp <- list.files(here("upload", "epi"), pattern = paste("PHL2", sample_type_acronym, "sequencing_results.csv", sep = "_"), recursive = TRUE, full.names = TRUE)
...
dir.create(here("upload", "freyja"), recursive = TRUE, showWarnings = FALSE)
