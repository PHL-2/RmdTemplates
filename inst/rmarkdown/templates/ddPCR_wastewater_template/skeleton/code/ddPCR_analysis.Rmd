...
```{r child = 'code/cluster_graph.Rmd', eval = ampExecute}
```
...
if (variance_test) {
  ...
  var_pander <- d_sd %>%
    select(Well, Sample_date, Type, Process, Target, conc = `Conc(copies/uL)`, avg_conc, cv_conc, sd, med_conc = median_conc, MAD = outlier_median, GESD_q
           ) %>%
...
d %>% 
  ...
  mutate(posDrops10k = overall_positives / Droplets * 100) %>%
  ungroup() %>%
  select(Well, Sample, Sample_date, Type, Process, posDrops10k) %>%
  #filter(Spiked == "Spiked") %>%
  ggplot(aes(x=Well, y=posDrops10k, fill = Process)) +
...
d %>%
  mutate(SampleType = ifelse(grepl(pattern = "East|West", x = Type) & Spiked == "Unspiked", paste("Neg", icInfo), SampleType),
         SampleType = ifelse(Type == "oldWW", "NegCtrl", SampleType)) %>%
   # filter(Spiked == "Spiked") %>%
  ggplot(aes(x=Type, y=posDrops10k, fill = Target)) +
  geom_boxplot(position = "dodge") +
  geom_point(
    aes(fill = Target),
    shape = 21,
