...
seqsender <- s_toPlot %>%
  filter4report() %>%
  mutate(genbank_name = paste0("SARS-CoV-2/Human/USA/", sample_id, "/", str_sub(project_name, 1, 4))) %>%
  mutate(sra_name = paste0("SARS-CoV-2/Human/USA/", sample_id)) %>%
  #GISAID name format needs to be hCoV-19/country/state-[sample_id without a year]/year
  mutate(gisaid_name = paste0("hCoV-19/USA/PA-", gsub(str_sub(project_name, 1, 4), "", sample_id), "/", str_sub(project_name, 1, 4))) %>%
  ...
  mutate(location = "USA: Pennsylvania",
         gisaid_location = "North America/USA/Pennsylvania",
         zip = "19146",
         type = "betacoronavirus",
         passage = "Original") %>%
  mutate(host_age_bin = ifelse(is.na(host_age_bin), "Unknown", host_age_bin),
         gisaid_age_range = ifelse(host_age_bin == "Unknown", host_age_bin, paste0("Age: ", host_age_bin)),
...
epi_report <- epi_2_upload_fp %>%
  ...
  merge(select(s, sample_id, sample_name), by.x = "accession_number", by.y = "sample_name", all.x = TRUE) %>%
...
test_upload <- TRUE #change this to FALSE when SRA upload and gisaid upload testing is successful and you want to submit the files for real
...
report_or_overwrite <- TRUE #change this to FALSE to overwrite the submitted files and to resubmit

if(report_or_overwrite) {
  overwrite_setting <- ""
} else{
  overwrite_setting <- "--overwrite"
}

seqsender_py <- file.path(seqsender_fp, "seqsender.py")

### generate files to upload; this block doesn't need to be run if you are going to submit because the next code block will also prepare the files for submission
# cli_submit("python",
#            seqsender_py,
#            c(proxy_setting, "prep",
#              "--unique_name", project_name,
#              "--config", shQuote(seqsender_config_fp, type = "cmd"),
...
cli_submit("python",
           seqsender_py,
           c(proxy_setting, "submit",
             "--unique_name", project_name,
             "--config", shQuote(seqsender_config_fp, type = "cmd"),
             "--metadata", shQuote(seqsender_meta_fp, type = "cmd"),
             "--fasta", shQuote(fasta_fp, type = "cmd"),
             upload_test_flag, overwrite_setting),
           shQuote_type = "cmd")

# ### submit files to genbank
cli_submit("python",
           seqsender_py,
           c(proxy_setting, "genbank",
             "--unique_name", project_name,
             "--config", shQuote(seqsender_config_fp, type = "cmd"),
             upload_test_flag, overwrite_setting),
           shQuote_type = "cmd")
