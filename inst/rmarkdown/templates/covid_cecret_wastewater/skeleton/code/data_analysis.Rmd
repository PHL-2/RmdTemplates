...
s %>%
  ...
  ungroup() %>%
  select(`Collect date` = sample_collect_date, `Samples/sites` = "sample_group", n, Merged, Good, Poor) %>%
  unique() %>%
...
freyja_average %>%
  group_by(sample_group, sample_collect_date, isMerged, CDC_lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund)) %>%
  ungroup() %>%
  {
  ggplot(., aes(x = isMerged, y = avg_rel_abund, fill = CDC_lineages)) +
    geom_col() +
    facet_wrap(~sample_collect_date+sample_group) +
    scale_fill_manual(values = 
                        c(colorRampPalette(brewer.pal(11, "Spectral"))(length(unique(.$CDC_lineages))-3),
...
freyja_table_delim <- freyja_average %>%
  group_by(sample_collect_date, sample_group, isMerged, CDC_lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(avg_rel_abund >= 0.01) %>%
  select(sample_collect_date, sample_group, isMerged, CDC_lineages) %>%
  unique() %>%
  arrange(sample_collect_date, sample_group, isMerged) %>%
  group_by(sample_collect_date, sample_group) %>%
...
freyja_average %>%
  group_by(sample_collect_date, mean_cov, sample_group, isMerged, CDC_lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(avg_rel_abund >= 0.01) %>%
  mutate(relative_abundance = ifelse(!is.na(total_sd),
                                  paste0(round(avg_rel_abund*100, 2), "% +/- ",
                                         round(total_sd*100, 2), "%"),
                                  paste0(round(avg_rel_abund*100, 2), "%")),
         sample_group_label = paste0(sample_collect_date, " ", sample_group, "\n<", isMerged, ">"),
         good_cov = mean_cov >= 0.6,
         mean_cov = paste0(round(mean_cov*100, 2), "%")) %>%
  arrange(sample_collect_date, sample_group, sample_group_label, CDC_lineages) %>%
  select(`Sample group` = sample_group_label, `Mean coverage` = "mean_cov",
         Lineage = "CDC_lineages", `Relative abundance` = "relative_abundance", good_cov) %>%
  unique() %>%
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      mutate(`Sample group` = cell_spec(`Sample group`, "latex",
                                        color = case_when((grepl("Merged sample", `Sample group`) &
                                                             !grepl("^A$|^B$", Lineage) &
                                                             good_cov) ~ "#4CBB17",
                                                          TRUE ~ "black"))) %>%
      select(-good_cov) %>%
...
freyja_table_delim <- freyja_average %>%
  group_by(sample_collect_date, sample_group, isMerged, CDC_lineages, lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(lineages != "Unresolved",
         ...
         avg_rel_abund >= 0.01) %>%
  select(sample_collect_date, sample_group, isMerged, lineages) %>%
  unique() %>%
  arrange(sample_collect_date, sample_group, isMerged) %>%
  group_by(sample_collect_date, sample_group) %>%
  summarize(total_samples = n()) %>%
...
freyja_average %>%
  group_by(sample_collect_date, mean_cov, sample_group, isMerged, CDC_lineages, lineages) %>%
  summarize(avg_rel_abund = sum(avg_rel_abund),
            total_sd = sum(total_sd)) %>%
  ungroup() %>%
  filter(lineages != "Unresolved",
  ...
  mutate(relative_abundance = ifelse(!is.na(total_sd),
                                  paste0(round(avg_rel_abund*100, 2), "% +/- ",
                                         round(total_sd*100, 2), "%"),
                                  paste0(round(avg_rel_abund*100, 2), "%")),
         sample_group_label = paste0(sample_collect_date, " ", sample_group, " <", isMerged, ">"),
         mean_cov = paste0(round(mean_cov*100, 2), "%")) %>%
  arrange(sample_collect_date, sample_group, sample_group_label, lineages) %>%
  select(`Sample group` = sample_group_label, `Mean coverage` = "mean_cov",
         Lineage = "lineages", `Relative abundance` = "relative_abundance") %>%
  unique() %>%
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      select_all(~gsub(" ", "\n", .)) %>%
