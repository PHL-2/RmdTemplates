...
epi_report %>%
  write_csv(here("upload", "epi", paste(project_name, "PHL_sequencing_results.csv", sep = "_")))

# Export the data for GISAID upload
gisaid <- s_toPlot %>%
  filter4report() %>%
  mutate(`Virus name` = paste0("hCoV-2/USA/", sample_id, "/", str_sub(project_name, 1, 4))) %>%
  ...
  mutate(`Collection date` = case_when((!is.na(sample_collection_date) & sample_collection_date != "NA") ~ sample_collection_date,
                                       (!is.na(PHL_sample_received_date) & PHL_sample_received_date != "NA") ~ PHL_sample_received_date,
                                       TRUE ~ sequencing_date)) %>%
  rename(Gender = "gender", Coverage = "median_coverage", `Originating lab` = "sample_collected_by", `Specimen source` = "sample_type") %>%
...
gisaid %>%
  write_csv(here("upload", "gisaid", paste(project_name, "PHL2_GISAID_upload.csv", sep = "_")))

# Export the data for SRA upload
bio_sample <- s_toPlot %>%
  filter4report() %>%
  select(-c(sample_name)) %>%
  mutate(isolate = paste0("SARS-CoV-2/USA/", sample_id, "/", str_sub(project_name, 1, 4))) %>%
  mutate(GISAID_virus_name = paste0("hCoV-2/USA/", sample_id, "/", str_sub(project_name, 1, 4))) %>%
  ...
  mutate(collection_date = case_when((!is.na(sample_collection_date) & sample_collection_date != "NA") ~ sample_collection_date,
                                       (!is.na(PHL_sample_received_date) & PHL_sample_received_date != "NA") ~ PHL_sample_received_date,
                                       TRUE ~ sequencing_date)) %>%
  rename(sample_name = "sample_id", collected_by = "sample_collected_by",
...
bio_sample %>%
  write_csv(here("upload", "sra", paste(project_name, "PHL2_BioSample_upload.csv", sep = "_")))
...
sra %>%
  write_csv(here("upload", "sra", paste(project_name, "PHL2_SRA_upload.csv", sep = "_")))
