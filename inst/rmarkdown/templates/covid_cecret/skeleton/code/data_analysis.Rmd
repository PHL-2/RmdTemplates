...
s_toPlot %>%
  ...
  mutate(fraction = 1/n(),
         sample_type = gsub(pattern = "(Mock).*", replacement = "\\1", x = sample_type),
         nextclade_clade_who = str_to_title(nextclade_clade_who)) %>%
  ungroup() %>%
  ...
  ggplot(., aes(x=sample_type, y=props, fill=variants)) +
    ...
    facet_wrap(sample_type+nextclade_clade_who~.,  scales="free", #space = "free",
               #labeller = labeller(.default = function(x) {gsub(" ", "\n", x)}, .multi_line = FALSE)
               ) +
