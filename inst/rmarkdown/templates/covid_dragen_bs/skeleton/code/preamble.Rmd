...
```{r upload data to basespace and run the app, message=FALSE, eval = dl_n_import_data}
...
bs_cli_create_fp <- here("code", "bs_cli_commands", "bs_cli_1_create_project.sh")
project_id <- bs_cli_submit(sh_exe_fp, bs_cli_create_fp, project_name)

### 2 - upload the fastq files
bs_cli_upload_fp <- here("code", "bs_cli_commands", "bs_cli_2_upload_fastq.sh")
...
bs_cli_app_ver_fp <- here("code", "bs_cli_commands", "bs_cli_3_get_app_version.sh")
dragen_app_version <- bs_cli_submit(sh_exe_fp, bs_cli_app_ver_fp, shQuote(app2run, type = "sh"))

### 4 - launch the app
bs_cli_launch_fp <- here("code", "bs_cli_commands", "bs_cli_4_launch_app.sh")
...
bs_cli_await_fp <- here("code", "bs_cli_commands", "bs_cli_5_await.sh")
awaiting <- bs_cli_submit(sh_exe_fp, bs_cli_await_fp, app_session_id)

### 6 - download data
bs_cli_download_fp <- here("code", "bs_cli_commands", "bs_cli_6_download.sh")
downloading <- bs_cli_submit(sh_exe_fp, bs_cli_download_fp, c(project_id,
                                               shQuote(here("data", "processed_bs"), type = "sh")))

### 7 - get app date
bs_cli_get_date_fp <- here("code", "bs_cli_commands", "bs_cli_7_get_date.sh")
...
```{r copy run folder files, eval = dl_n_import_data}
...
```{r save fastq and fasta for upload, eval = dl_n_import_data}
...
if(length(s_upload) > 0) {
  ...
  fasta2upload %>%
    ...
    ungroup() %>%
    select(-c(FileName)) %>%
    write_delim(file = here("upload", "fasta", paste0(project_name, "_PHL2_combined.fasta")), delim = "", col_names = FALSE)
