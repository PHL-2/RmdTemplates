...
```{r, fig.width=10, fig.height=5, eval = if(!has_extra){FALSE}else{has_CT}}
...
```{r, fig.width=10, fig.height=5, eval = if(!has_extra){FALSE}else{has_RLU}}
...
```{r, eval = if(!has_extra){FALSE}else{has_CT}}
empty_plate %>%
...
```{r, eval = if(!has_extra){FALSE}else{has_RLU}}
empty_plate %>%
  merge(s, by = c("plate_coord", "plate_row", "plate_col", "plate"), all = TRUE) %>%
  filter(!is.na(plate_coord)) %>%
  droplevels() %>%
...
s_toPlot %>%
  select(sample_id, Conflict, Note, sample_type, nc_QC_status) %>%
  ...
  filter(!grepl("scorpio found insufficient support", variant2_prop)) %>%
  ...
  mutate(sample_id = factor(sample_id, levels = unique(sample_id[order(nc_QC_status, variants, decreasing = FALSE)]))) %>%
...
s_toPlot %>%
  ...
  ggplot(aes(x = nc_QC_status, y = cov_percent)) +
    geom_boxplot(outlier.alpha = 0) +
    geom_point(aes(fill = coverage, shape = sample_type), position = position_jitterdodge(jitter.width = 1), size = 3) +
    ...
    guides(fill = guide_legend(reverse=T, order = 1, override.aes = list(shape = 21)))
