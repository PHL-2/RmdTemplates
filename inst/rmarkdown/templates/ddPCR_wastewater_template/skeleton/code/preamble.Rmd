...
useExportFolder <- !any(ampCheck, standardCheck)
...
copy_path = list.files(path = here(), pattern = "Analyzed.csv", recursive = TRUE, full.names = TRUE)

source(file = file.path(here(), "code","scripts","standard_input.R")) #reads in analyzed csv and creates d_pre
...
copy_path = list.files(
  path = here(), 
  pattern = "Amplitude.csv", 
  recursive = TRUE, 
  full.names = TRUE)

source(file = file.path(here(), "code", "scripts", "clustering_algorithm.R")) #reads in amplitude data, clusters data, then creates d_pre

```

```{r}

targetsInfo = d_pre$Target %>%
  unique()
...
d_var <- d %>%
  group_by(Sample, Spiked, Target, Sample_received) %>%
...
d_sd <- d %>%
  left_join(d_var, by = c("Sample", "Spiked", "Target", "Sample_received")) %>%
...
if (variance_test | signal_check) {
  wellsToRemove1 <- d_sd %>%
    filter(outlier_median >= 9| GESD_q <= 0.01, 
           grepl(pattern = "NorthEast|SouthEast|SouthWest", x = Sample),
           Spiked == "Spiked",
           grepl(pattern = paste(targetsInfo, collapse = "|"), x = Target)) %>%
    group_by(Sample, Spiked, Sample_received) %>%
...
ww_conc_result_summarised <- d %>%
  ...
  ungroup() %>%
  select(Sample, Target, Sample_received, stock_conc_GCuL, stock_conc_GC1L) %>%
  arrange(Sample_received, Sample, Target)
...
dControls <- d %>%
  filter(!grepl(pattern = "NorthEast|SouthEast|SouthWest", x = Sample)) %>%
  mutate(exp_con = ifelse(grepl(pattern = "PCR", x = Sample ,ignore.case = T), "ddPCR control", "Extraction control"))
...
### To be deleted?
# threshold <- dNeg %>%
  # group_by(Sample, Process, Target, Sample_received) %>%
  # summarize(
  #   n_samp = n(),
...
#   filter(grepl(pattern = "NorthEast|SouthEast|SouthWest", x = Sample),
#          Process == "Concentrated") %>%
#   group_by(Sample, Process, Spiked, Target, Sample_received) %>%
#   summarise(avg_pos_droplets = mean(Positives),
#             avg_conc = mean(`Conc(copies/uL)`),
#             sd_conc = sd(`Conc(copies/uL)`)) %>%
#   mutate(stock_conc = stock_concentration_calc(concentration = avg_conc, 
...
#   group_by(Sample, Process, Target, Spiked, Sample_received) %>%
#   summarise(avg_pos_droplets = mean(Positives),
#             avg_conc = mean(`Conc(copies/uL)`),
#             sd_conc = sd(`Conc(copies/uL)`)) %>%
#   mutate(stock_conc = stock_concentration_calc(concentration = avg_conc, 
...
ww_conc_result <- d %>%
  filter(#Process == "Concentrated",
         #Spiked == "Spiked",
         Target != icInfo,
         Sample == "NorthEast" | Sample == "SouthEast" | Sample == "SouthWest") %>%
  #left_join(sc2_n2, by = "Sample_received") %>%
  ...
  ungroup() %>%
  select(Well, Sample, Target, Sample_received, stock_conc_GCuL, stock_conc_GC1L) %>%
  arrange(Sample_received, Sample, Target)
...
LIMS <- d_sd %>%
  mutate(flag = ifelse((Sample == "NorthEast" | Sample == "SouthEast" | Sample == "SouthWest") & (outlier_median > 9) | Droplet_count < 10000, TRUE, FALSE),
         flag = ifelse(is.na(GESD_q), FALSE, flag),
         flagComment = case_when((outlier_median > 9) & flag ~ "outlier",
                                 Droplet_count < 10000 ~ "<10000 droplets",
                                 #(Target == "MHV" & Droplet_count < 100) ~ "Internal control failed"
                                 ),
         uniq_sample_name = paste0("WW-", Sample_received, "-", Sample),
         #Pos10k = percentage_positive*100,
         rxn_vol_uL = rxn_vol_uL, 
         input_vol_uL = input_vol_uL, 
         extraction_vol_uL = extraction_vol_uL, 
         elution_vol_uL = elution_vol_uL,
         end_concentration_vol_uL = end_concentration_vol_uL,
         init_concentration_vol_uL = init_concentration_vol_uL
         ) %>%
  right_join(d_pre) %>%
  select(Well, uniq_sample_name, Sample, Sample_received, Target, `Conc(copies/uL)`, Droplet_count, Positives, Negatives, pos10k, avg_conc, sd_conc, rxn_vol_uL, input_vol_uL, extraction_vol_uL, elution_vol_uL, end_concentration_vol_uL, init_concentration_vol_uL, outlier_median, flag, flagComment) %>%
  arrange(Well)


write_csv(LIMS, file = file.path(here(), "LIMSexport.csv"))
...
towrite <- ww_conc_result_summarised %>%
  ungroup() %>%
  select(pcr_gene_target = Target, pcr_target_avg_conc = stock_conc_GC1L, sample_group = Sample, sample_received_date = Sample_received) %>%
...
sequencing_metadata <- d %>%
  filter(grepl(pattern = "oldWW|Zepto|Twist|NorthEast|SouthEast|SouthWest", x = Sample),
         #(Process == "Concentrated" | grepl("oldWW", Sample)),
         !(Well %in% wellsToRemove),
  ...
  mutate(Target = case_when(concentration == "avg_conc" ~ paste0(Target, "_gc/uL"),
                            ...
                            TRUE ~ NA),
         sample_collect_date = Sample_received - days_between_collection,
         suffix = ifelse(grepl("oldWW|ZeptoSC2", Sample), Sample, gsub("[a-z]", "", Sample)),
         uniq_sample_name = paste("WW", Sample_received, Sample, sep = "-"),
  ...
  select(uniq_sample_name, `bs-ww_surv_system_sample_id`, sample_group = "Sample",
         sample_collect_date, sample_received_date = "Sample_received",
