...
```{r settings for processing the sequencing run}
...
ssh_seqsender_cmd <- function(pos_argument,
  ...
  if (pos_argument == "prep") {
    additional_flags <- paste("-bsng", additional_flags)
    success_msg <- c("here", success_msg)
...
gisaid_meta_fp <- here("gisaid", paste(project_name, "gisaid.csv", sep = "_"))

#path of local seqsender config file
seqsender_config_fp <- list.files(aux_seqsender_fp, pattern = "seqsender_ns_login_config.yaml", full.names = TRUE)
...
#creating a separate GISAID metadata file until seqsender fixes issue
dir.create(here("gisaid"))

gisaid_metadata <- seqsender %>%
  rename(`gs-covv_virus_name` = `gs-sample_name`,
         `gs-covv_collection_date` = collection_date) %>%
  select(starts_with("gs-")) %>%
  rename_at(vars(starts_with("gs-")), ~ gsub("gs-", "", .)) %>%
  mutate(submitter = gisaid_username,
         fn = NA,
         covv_authors = authors)

gisaid_metadata %>%
  write_csv(gisaid_meta_fp, na = "")
...
if(test_upload) {
  message("\n*****")
  message("Before doing a real submission, upload ", basename(consensus_fasta_fp), " from folder ")
  message(here("upload", "fasta/"))
  message("to https://clades.nextstrain.org/\n")
  message("Check that the FASTA sequences are acceptable (no novel early stop codons, novel truncated spike proteins, or novel frameshifts)")
  message("If samples are low quality, remove them from the submission by adding their SampleIDs to the variable 'failed_sample_fasta' defined above")
  message("*****")
  Sys.sleep(20)
}
...
upload_gisaid_metadata <- system2("scp", c(gisaid_meta_fp,
                                           paste0(ec2_hostname, ":", ec2_upload_tmp_fp, "/submission_files/GISAID/")))
                                  
ssh_command_check(upload_gisaid_metadata)
...
if(all(grepl("^EPI_ISL_0$", seqsender$`bs-gisaid_accession`)) | all(is.na(seqsender$`bs-gisaid_accession`))) {
  ...
  run_in_terminal(paste("ssh -tt", ec2_hostname,
                        shQuote(paste(covCLI_cmd, "upload",
                                      ...
                                      "--metadata", paste(ec2_upload_tmp_fp, "submission_files", "GISAID", basename(gisaid_meta_fp), sep = "/"),
