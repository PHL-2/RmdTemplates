...
s %>%
  ...
  mutate(n = n() - sum(grepl("Merged", ww_group)),
         ...
         Good = sum(pct_genome_coverage_over_10x >= 0.6, na.rm = TRUE),
         Poor = n + sum(grepl("Merged", ww_group)) - Good) %>%
  ungroup() %>%
  select(sample_group, n, Merged, Good, Poor) %>%
  unique() %>%
...
freyja_average %>%
  {
  ggplot(., aes(x = sample_group, y = avg_rel_abund, fill = combined_lineage)) +
    geom_col() +
...
s_toPlot %>%
  filter4report() %>%
  group_by(sample_collection_date) %>%
  mutate(all_samples_n = n()) %>%
  ungroup() %>%
  select(ww_group, sample_collection_date, Nextclade_pango, nextclade_clade_who, all_samples_n) %>%
  group_by(sample_collection_date, ww_group, nextclade_clade_who, Nextclade_pango) %>%
  ...
  ungroup() %>%
  arrange(sample_collection_date, -variant_percentages) %>%
  select(-variant_percentages) %>%
  unique() %>%
  rename(`Received date` = "sample_collection_date", `Samples and Sites` = "ww_group", `WHO` = "nextclade_clade_who", `Variant` = "Nextclade_pango", `Variant percentage` = "variant_percentages_text") %>%
  { if(use_kable)
...
has_pos_ctrl <- s %>%
  filter(grepl("SC2-Spiked", sample_name)) %>%
...
Number of samples to upload to GISAID database: `r nrow(all_sample_ids_from_dcipher[all_sample_ids_from_dcipher$file == "gisaid_name", "sample_id"])` \newline
Number of samples to upload to SRA: `r nrow(all_sample_ids_from_dcipher[all_sample_ids_from_dcipher$file == "sra_name", "sample_id"])` \newline
Number of samples to upload to Genbank: `r nrow(all_sample_ids_from_dcipher[all_sample_ids_from_dcipher$file == "genbank_name", "sample_id"])`\newline
