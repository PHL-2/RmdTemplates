...
unaccounted_sample_types <- sort(unique(s_meta$ww_group)[!unique(s_meta$ww_group) %in% c(ctrl_fct_lvls)])
...
cecret_aggregate <- here("data", "processed_cecret", "cecret_results.csv") %>%
  ...
  rename(sample_id = "sample", vadr_passfail = "vadr_p/f") %>%
  as.data.frame()
...
cecret_aggregate %>%
  ...
  mutate(software = case_when(grepl("^PUSHER-", version) ~ "PUSHER",
         ...
         version = gsub("^PUSHER-|^PANGO-", "", version)) %>%
  mutate(pipeline = "UPHL-BioNGS-Cecret") %>%
  select(pipeline, software, version) %>%
  write_csv(file = software_version_fp, append = TRUE)
...
s <- s_meta %>%
  ...
  mutate(ww_group = case_when(grepl("^Undetermined$", sample_id) ~ "Unassigned reads",
                              (grepl("Merged", sample_id) & !isControl) ~ paste0(ww_group, " - Merged"),
                              (pct_genome_coverage_over_10x < 0.6 & !isControl) ~ paste0(ww_group, " - Poor"),
                              TRUE ~ ww_group),
  ...
  order_on_other_col(sample_group, ww_group, decreasing = FALSE) %>%
  #order the sample_id by sample type then by plate position
  mutate(sample_id = factor(sample_id, levels = unique(sample_id[str_order(as.numeric(sample_group), decreasing = FALSE, numeric = TRUE)]))) %>%
  arrange(sample_id) %>%
  droplevels()
...
s_toPlot <- s %>%
  ...
  filter(coverage_qc_status == "Good") %>%
  filter(ww_group != "Wastewater sample") %>%
  mutate(sample_id = as.character(sample_id)) %>%
  droplevels()

has_N2 <- any(grepl("^N2_cp-uL$", colnames(s)))
...
freyja_grouping <- s %>%
  select(sample_id, sample_name, sample_group, ww_group, sample_collection_date,
  ...
  mutate(combined_lineage = stringi::stri_sub(lineages, 1, substring_end),
         combined_lineage = gsub("\\.$", "", combined_lineage),
         combined_lineage = factor(combined_lineage, levels = str_sort(unique(combined_lineage), numeric = TRUE)),
         relative_abundances = ifelse(is.na(relative_abundances) & grepl("Unresolved", combined_lineage),
                                      1, relative_abundances),
         lineages = gsub("\\.$", "", lineages))

freyja_matrix <- freyja_grouping %>%
  complete(nesting(sample_id, ww_group), combined_lineage, fill = list(relative_abundances = 0)) %>%
...
freyja_average <- freyja_grouping %>%
  filter(!isControl) %>%
  filter(!grepl("Merged", ww_group)) %>%
  droplevels() %>%
  complete(nesting(sample_id, sample_group), nesting(combined_lineage, main_lineage), fill = list(relative_abundances = 0)) %>%
  group_by(sample_id, sample_group, combined_lineage, main_lineage) %>%
  summarize(relative_abundances = sum(relative_abundances)) %>%
  ungroup() %>%
  ...
  group_by(sample_group, combined_lineage, main_lineage) %>%
...
epi_report_fp <- here("upload", "epi", paste0(project_name, "_PHL2_sequencing_results.csv"))

#to do: merge lineages into the ones reported by CDC

freyja_grouping %>%
  filter(sample_id %in% s_toPlot$sample_id,
         relative_abundances > 0.01) %>%
  select(ww_group, sample_name, sample_collection_date,
         pct_genome_coverage_over_10x, lineages, relative_abundances) %>%
  arrange(ww_group, sample_name, desc(relative_abundances)) %>%
  write_csv(epi_report_fp)

```

```{r load data submission metadata, eval = submitting_data}
...
dcipher_meta_fp <- here("upload", "dcipher", paste0(project_name, "_PHL2_dcipher_upload.tsv"))
fasta_fp <- here("upload", "fasta", paste0(project_name, "_PHL2_combined.fasta"))

#read in data submission metadata
ww_submission_metadata_fp <- file.path(dirname(here()), "aux_files", "data_submission", "dcipher")
ww_sample_metadata_fp <- file.path(ww_submission_metadata_fp, "nwss_wastewater_metadata.csv")
data_submit_metadata_fp <- file.path(ww_submission_metadata_fp, "wastewater_data_submission_metadata.csv")

ww_sample_metadata <- read_csv(ww_sample_metadata_fp)
data_submit_metadata <- read_csv(data_submit_metadata_fp) %>%
  select(common_field, value) %>%
  pivot_wider(names_from = "common_field", values_from = "value")

```

```{r export data for reporting and submission, eval = submitting_data}
...
samp_report <- s %>%
  ...
  mutate(sample_collection_date = case_when(!(is.na(sample_collection_date) | as.character(sample_collection_date) == "") ~ as.character(sample_collection_date),
                                           TRUE ~ NA),
         sample_collection_date = as.Date(sample_collection_date, tryFormats = c("%Y-%m-%d", "%m/%d/%Y", "%m/%d/%y")),
         sample_collection_year = format(sample_collection_date, format = "%Y"),
         #create db names
         ...
         gisaid_name = paste0("hCoV-19/env/USA/PA-", gsub(str_sub(sequencing_date, 1, 4), "", sample_id), "/", sample_collection_year),
         biosample_name = paste0("SARS-CoV-2/Wastewater/USA/", sample_id, "/", sample_collection_year),
         specimen_processing_id = sample_name,
         
         #need to fill these in
         ww_surv_target_1_known_present = "???", #this should be yes or no, depends on the ddPCR results
         ww_surv_target_1_conc = "???", #this is the ddPCR transformed unit
         ww_surv_target_2_known_present = "???", #this should be yes or no, depends on the ddPCR results
         ...
         description = paste(sample_description, "Viral sequencing was performed following a tiling amplicon strategy using the ARTIC", artic_primer_scheme, "primer scheme. Sequencing was performed using the Illumina", instrument_type, "instrument with 2x", read_length, " bp chemistry. Libraries were prepared using Illumina COVIDSeq Test kit."),
         gisaid_accession = "")

samp_report %>%
  write_tsv(dcipher_meta_fp)

#additional items to add
#population_served (NWSS) -> ww_population (BioSample)
#wwtp_name (NWSS) -> collection_site_id (BioSample)
#is lod_sewage the same as ww_flow?
#ww_surv_system_sample_id (may need to get this id from the NWSS submmission process)
...
dcipher_2_upload_fp <- list.files(here("upload", "dcipher"), pattern = ".tsv", recursive = TRUE, full.names = TRUE)

all_sample_ids_from_dcipher <- dcipher_2_upload_fp %>%
  read_tsv() %>%
  select(sample_id, biosample_name, gisaid_name) %>%
  ...
  pivot_longer(cols = c("biosample_name", "gisaid_name"), names_to = "file", values_to = "upload_id") %>%
  mutate(upload_id = gsub(".*/USA/|/.*", "", upload_id)) %>%
  mutate(all_sample_same = upload_id == sample_id)

if (!all(all_sample_ids_from_dcipher$all_sample_same)) {
  stop (simpleError("Some of the database names don't match the sample_id names! The GenBank name may also be missing because it didn't pass vadr check. Try submitting the GenBank files anyway through dcipher to see if it passes"))
}

all_sample_ids_from_dcipher %<>% select(file, upload_id) %>%
...
sample_type_color_select <- c(15, 19, 16, 13, 2, #colors for sequencing controls
                              11, 14, 12, #colors for wastewater controls
...
sample_group_legend <- s %>%
  select(sample_id, ww_group, sample_group) %>%
  ...
  ggplot(., aes(y = 1, x = sample_id, fill = ww_group)) +
    ...
    scale_x_discrete(labels = .$sample_group) +
    theme_bw() +
    theme(
