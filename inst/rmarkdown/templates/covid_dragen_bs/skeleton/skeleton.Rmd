...
source('metadata_template/generate_barcodes_IDT.R')
...
```{r data export}
dir.create(here(prj_dir, "upload", "epi"), recursive = TRUE)
dir.create(here(prj_dir, "upload", "gisaid"), recursive = TRUE)
dir.create(here(prj_dir, "upload", "sra"), recursive = TRUE)
...
epi_report %>%
  write_csv(here(prj_dir, "upload", "epi", paste(sequencing_date, "COVIDSeq_PHL_sequencing_results.csv", sep = "_")))
...
gisaid <- s_toPlot %>%
  ...
  #filter out mediocre results with lower coverages
  filter(!(nc_QC_status == "mediocre" & coverage_num < 80)) %>%
  mutate(`Virus name` = paste0("hCoV-2/USA/", sample_id, "/", str_sub(sequencing_date, 1, 4))) %>%
...
gisaid %>%
  write_csv(here(prj_dir, "upload", "gisaid", paste(sequencing_date, "COVIDSeq_PHL2_GISAID_upload.csv", sep = "_")))
...
bio_sample <- s_toPlot %>%
  ...
  mutate(isolate = paste0("SARS-CoV-2/USA/", sample_id, "/", str_sub(sequencing_date, 1, 4))) %>%
  mutate(GISAID_virus_name = paste0("hCoV-2/USA/", sample_id, "/", str_sub(sequencing_date, 1, 4))) %>%
...
bio_sample %>%
  write_csv(here(prj_dir, "upload", "sra", paste(sequencing_date, "COVIDSeq_PHL2_BioSample_upload.csv", sep = "_")))
...
sequencing_date <- sort(list.dirs(here("Runs"), recursive = FALSE, full.names = FALSE), decreasing = TRUE)
sequencing_date <- sequencing_date[!grepl("test_run", sequencing_date)][1]
prj_dir <- here("Runs", sequencing_date)

project_name <- basename(here())

qc_report_fp <- paste0(project_name, ".Rmd")
qc_output_fp <- paste(sequencing_date, "_COVIDSeq.QC.report_gen.on.", Sys.Date(), ".pdf", sep="")

rmarkdown::render(qc_report_fp, output_file = here(prj_dir, qc_output_fp))
rmarkdown::render(qc_report_fp, output_file = here(prj_dir, "README.md"),
