...
s %>%
  ...
  mutate(n = n(),
         Good = ifelse(!isControl, as.character(n - sum(grepl("Poor", ww_group))), ""),
         Poor = ifelse(!isControl, as.character(sum(grepl("Poor", ww_group))), ""),
         Merged = ifelse(!isControl, as.character(sum(grepl("Merged", ww_group))), "")) %>%
  ungroup() %>%
  select(sample_group, n, Good, Poor, Merged) %>%
  unique() %>%
  rename(`Samples/sites` = "sample_group") %>%
...
s %>%
  ggplot(aes(y=read_counts, x=sample_group)) +
    ...
    labs(
      ...
      y="Read counts",
      x="",
      fill = "Samples and Sites",
      size = "Samples and Sites"
    )
...
s %>%
  ...
  ggplot(., aes(x = "", y = percent_of_lane, fill=ww_group)) +
    ...
    facet_grid(sample_group~., scales = "free", space = "free", switch = "y") +
...
fastqc %>%
  ...
  ggplot(., aes(x=position, y=MeanQual, color = ww_group)) + 
    ...
    geom_point() +
    theme_bw() + 
    labs(x='Position in sequence read', y='Average quality score per sample type', color = 'Samples and Sites')
  }
...
s %>%
  filter(!is.na(`N2_cp-uL`)) %>%
  droplevels() %>%
  {
  ggplot(., aes(x = `N2_cp-uL`, y = read_counts, fill = ww_group)) +
    ...
    labs(y = "Read Counts", x = "N2 cp/uL", fill = "Site")
  }
...
s %>%
  ...
  droplevels() %>%
  {
  ggplot(., aes(x = `N2_cp-uL`, y = qubit_conc_ng_ul, fill = ww_group)) +
    geom_point(shape = 21, size = 3) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(.$ww_group), names(ann_geom_values$sample_type_colors))]) +
    theme_bw() + 
    guides(fill = guide_legend(override.aes = list(shape = 21))) +
...
empty_plate %>%
  ...
  ggplot(aes(x=plate_col, y=fct_rev(plate_row), fill = `N2_cp-uL`)) +
    geom_tile(fill = "white") +
    geom_point(shape = 21, size = 10) +
    facet_wrap(~plate, ncol = 1) +
    #if there is no covid quantity values, use the discrete fill
    {
      if(all(is.na(s$`N2_cp-uL`))) scale_fill_viridis_d(na.value = "white")
...
k2 %>%
  ...
  merge(select(s, sample_id, ww_group, sample_group), by = "sample_id", all = TRUE) %>%
...
k2 %>%
  ...
  ggplot(aes(x=k2_props, fill = ww_group)) +
    ...
    facet_grid(sample_group~., switch = "y") +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(s$ww_group), names(ann_geom_values$sample_type_colors))]) +
...
s %>%
  select(sample_id, ww_group, matches("pct_genome_coverage_over")) %>%
...
s %>%
  select(sample_id, ww_group, sample_group, matches("pct_s_gene_coverage_over")) %>%
  ...
  ggplot(., aes(x = depth, y = pct_coverage, fill = ww_group, group = sample_id)) +
    ...
    geom_point(shape = 21, size = 3) +
    scale_y_continuous(labels=percent) +
    facet_wrap(~sample_group, nrow = 2) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(.$ww_group), names(ann_geom_values$sample_type_colors))]) +
    scale_color_manual(values = ann_geom_values$sample_type_colors[match(levels(.$ww_group), names(ann_geom_values$sample_type_colors))]) +
    theme_bw() +
    theme(
...
s %>%
  select(ww_group, sample_group, starts_with("amp_tile")) %>%
...
s %>%
  select(ww_group, median_coverage, pct_genome_coverage_over_10x, coverage_qc_status) %>%
  filter(!is.na(median_coverage)) %>%
  mutate(coverage_qc_status = factor(coverage_qc_status, levels = c("Good", "Poor"))) %>%
  {
  ggplot(., aes(x = coverage_qc_status, y = pct_genome_coverage_over_10x)) +
    geom_hline(yintercept = 0.6, color = "grey", linetype = "dotted", lwd=0.75) +
    geom_boxplot(outlier.alpha = 0) +
    geom_point(aes(fill = median_coverage), shape = 21, position = position_jitter(), size = 3) +
...
s %>%
  select(sample_id, ww_group, nextclade_qc_overallstatus, nextclade_qc_overallscore, matches("^qc\\..*\\.score$")) %>%
  ...
  ggplot(., aes(y = sample_id, x = qc_score_percent, fill = `Nextclade QC score`)) +
    ...
    scale_y_discrete(breaks = .$sample_id, labels = .$ww_group) +
    theme_bw() +
    theme(
      strip.background = element_blank(),
      strip.text.y = element_text(angle = 0),
...
s_toPlot %>%
  select(sample_id, Nextclade_pango, ww_group, nextclade_clade_who) %>%
  group_by(ww_group, nextclade_clade_who) %>%
  mutate(fraction = 1/n()) %>%
  ungroup() %>%
  mutate(variants = fct_lump(Nextclade_pango, 20, w = fraction)) %>% 
  select(-Nextclade_pango) %>%
  group_by(ww_group, variants, nextclade_clade_who) %>%
  summarize(props = sum(fraction)) %>%
  ungroup() %>%
  mutate(variants = factor(variants, levels = names(ann_geom_values$variants))) %>%
  droplevels() %>%
  {
  ggplot(., aes(x=ww_group, y=props, fill=variants)) +
    geom_bar(stat="identity", position = position_fill(reverse = FALSE)) +
    facet_grid(~ww_group+nextclade_clade_who, space = "free", scales="free",
               labeller = labeller(.default = function(x) {gsub("$", "\n", x)}, .multi_line = TRUE)) +
...
### Figure showing different USHER placements through Pangolin

```{r multiple variant assignments, fig.height=9, fig.width=8}

s_toPlot %>%
  select(sample_id, pangolin_note, ww_group, nextclade_qc_overallstatus) %>%
  ...
  ggplot(., aes(x=props, y=sample_id, fill=variants)) +
    ...
    facet_grid(nextclade_qc_overallstatus+ww_group~., space="free", scales="free",
...
freyja_grouping %>%
  ...
  merge(select(s_toPlot, sample_id, pangolin_lineage, Nextclade_pango, ww_group, coverage_qc_status), by = c("sample_id", "ww_group"), all.y = TRUE) %>%
...
s_toPlot %>%
  ...
  ungroup() %>%
  select(ww_group, PHL_sample_received_date, Nextclade_pango, nextclade_clade_who, all_samples_n) %>%
  group_by(PHL_sample_received_date, ww_group, nextclade_clade_who, Nextclade_pango) %>%
  ...
  unique() %>%
  rename(`Received date` = "PHL_sample_received_date", `Samples and Sites` = "ww_group", `WHO` = "nextclade_clade_who", `Variant` = "Nextclade_pango", `Variant percentage` = "variant_percentages_text") %>%
  { if(use_kable)
      kable(., "latex", longtable = F, digits=2, booktabs=T, escape=F,
            col.names = linebreak(c("Sample\nreceived", "Samples and Sites", "WHO", "Variant", "Variant\npercentage"), align = "l")) %>%
...
Orange samples also will not be reported due to low quality (bad or mediocre NextClade QC with <80% 30X coverage) \newline
...
Number of samples to send to epidemiologists: \newline
...
`r if(length(rerun_samples) > 0) { paste0("\\textcolor{red}{\\huge These non-control samples that didn't go through the pipeline correctly!! Try rerunning them through the pipeline again!}")}`
...
s %>%
  ...
  rename(`sample/site` = "ww_group",
         `read counts` = "read_counts",
         lineage = "Nextclade_pango",
         `10X pct coverage` = "pct_genome_coverage_over_10x") %>%
  { if(use_kable) 
    mutate(., sample = gsub("_", "-", sample_id)) %>%
    mutate(sample = cell_spec(sample, "latex",
...
* Nextflow: `r software_version$Nextflow`
  ...
  + Cecret pipeline: `r software_version[["Cecret"]]`
    ...
    - Pangolin: `r software_version[["pangolin"]]`
      - UShER database: `r software_version[["PUSHER"]]`
...
freyja_grouping %>%
  group_by(sample_id, ww_group, combined_lineage) %>%
  ...
  ungroup() %>%
  complete(nesting(sample_id, ww_group), combined_lineage, fill = list(relative_abundances = NA)) %>%
...
freyja_matrix %>% 
  ...
  ggplot(aes(y=`Shannon diversity`, x=ww_group, fill = ww_group)) +
    geom_boxplot(outlier.alpha = 0) +
    geom_point(size = 2, shape = 21, position = position_jitter(width = 0.2)) +
    scale_y_continuous(labels = number_format(accuracy = 1)) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(s$ww_group), names(ann_geom_values$sample_type_colors))]) +
    theme_bw() + 
    theme(panel.grid = element_blank(),
    ...
    labs(
      y="Shannon diversity",
      x="", fill = "Samples and Sites", size = "Samples and Sites"
    )
...
s %>%
  ...
  ggplot(., aes(x=Axis.1, y=Axis.2)) +
    geom_point(aes(fill = ww_group, shape=21), size = 3) +
    stat_ellipse(mapping = aes(x=Axis.1, y=Axis.2, color = ww_group), inherit.aes = FALSE) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(s$ww_group), names(ann_geom_values$sample_type_colors))]) +
    ...
    labs(x=paste0("PCoA axis 1 (", pct[1], "%)"), 
         ...
         color="Samples and Sites", fill = "Samples and Sites", shape="Samples and Sites",
...
freyja_sample_type %>%
  {
  ggplot(., aes(x = ww_group, y = avg_rel_abund, fill = combined_lineage)) +
    geom_col() +
...
freyja_sample_type %>%
  ...
  pivot_wider(names_from = "ww_group", values_from = "print_rel_abund", values_fill = "") %>%
...
s %>%
  select(sample_id, ww_group, freyja_summarized) %>%
