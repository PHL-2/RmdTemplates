...
primer_select <- 2 # set variable as 1 for V3, 2 for V5.3.2
...
artic_primer_scheme <- c("ncov2019", "articv5.3.2")[primer_select]
...
if (is.na(folder_date)) {
  stop(simpleError(paste0("Could not find the raw run folder in ", sequencing_folder_fp)))
}
...
```{r prepare the run folders for upload, message = NA, eval = run_dragen_app}
...
for(who_label in c("Reference", sort(unique(s$clade_who)[unique(s$clade_who) != "Reference"]))) {
  ...
  for(combined_variant in combined_list) {
    ...
    if(grepl("[0-9]$", combined_variant)) {
      ...
      combined_variant <- gsub("\\.", "\\\\.", combined_variant)
      exact_lineage_regex <- paste0("^", combined_variant, "$")
      sublineage_regex <- paste0("^", combined_variant, "\\.")
