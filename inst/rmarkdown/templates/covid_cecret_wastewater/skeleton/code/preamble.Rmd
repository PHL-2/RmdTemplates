...
remove_sample_number_regex <- "_S[0-9]+"
...
run_folder_name <- list.files(here("data", "processed_bclconvert"), pattern = "^[0-9]{6}_.*[0-9]+_[0-9]+_[0-9A-Z-]+$")
...
fastqc <- lapply(fastqc_fps, function(f) {
  ...
  read_qc$sample_id <- gsub(paste0(remove_sample_number_regex, "|.*/|_R[12].*"), "", f)
...
actual_read_lengths <- lapply(fastqc_fps, function(f) {
  ...
  read_in_lengths$sample_id <- gsub(paste0(remove_sample_number_regex, "|.*/|_R[12].*"), "", f)
...
samtools_stat <- lapply(samtools_stats_fps, function(f) {
  ...
  summary_numbers$sample_id <- gsub(paste0(remove_sample_number_regex, "|.*/|\\.stats\\.txt"), "", f)
...
mapped_read_lengths <- lapply(samtools_stats_fps, function(f) {
  ...
  if(line_end == 0) {
    read_lengths <- data.frame(sample_id = gsub(paste0(remove_sample_number_regex, "|.*/|\\.stats\\.txt"), "", f),
                               RL = "RL",
    ...
    read_lengths$sample_id <- gsub(paste0(remove_sample_number_regex, "|.*/|\\.stats\\.txt"), "", f)
  }
...
amplicon_depth <- amplicon_fps %>%
  ...
  mutate(sample_id = gsub(paste0(remove_sample_number_regex, "|\\.primertrim\\.sorted\\.bam"), "", sample_id)) %>%
...
slide_window_coverage <- coverage_fps %>%
  ...
  mutate(sample_id = gsub(".*/", "", FileName),
         sample_id = gsub(paste0(remove_sample_number_regex, "|\\.depth\\.txt"), "", sample_id)) %>%
  select(-c(FileName)) %>%
  group_by(sample_id) %>%
...
cov_stats <- cov_num_fps %>%
  ...
  mutate(sample_id = gsub(remove_sample_number_regex, "", sample_id)) %>%
...
freyja <- freyja_fps %>%
  ...
  mutate(sample_id = gsub(".*/", "", FileName),
         sample_id = gsub(paste0(remove_sample_number_regex, "|_demix.tsv"), "", sample_id),
...
snpdists <- snpdists_raw_fp %>%
  ...
  rename(sample_id = 1) %>%
  mutate(sample_id = gsub(remove_sample_number_regex, "", sample_id)) %>%
  rename_all(.funs = function(x) gsub(remove_sample_number_regex, "", x)) %>%
...
list(cov_stats, coverage, s_gene_coverage, next_clade, freyja, amplicon_depth, samtools_stat, k2_sc2) %>%
  ...
  mutate(nextclade_clade_who = case_when((grepl("^B$|^B\\.[0-9]+", Nextclade_pango) & nextclade_clade == "19A") ~ "Reference", #Illumina PC
         ...
         sample_id = gsub(remove_sample_number_regex, "", sample_id),
...
dir.create(here("upload", "epi"), recursive = TRUE, showWarnings = FALSE)
epi_report_fp <- here("upload", "epi", paste(project_name, "PHL2", sample_type_acronym, "sequencing_results.csv", sep = "_"))
...
#write file for LIMS
dir.create(here("upload", "lims"), recursive = TRUE, showWarnings = FALSE)
lims_import_fp <- here("upload", "lims", paste(project_name, "PHL2", sample_type_acronym, "LIMS_import.csv", sep = "_"))
...
dir.create(here("upload", "seqsender"), recursive = TRUE, showWarnings = FALSE)
...
dir.create(here("data", "figures"), recursive = TRUE, showWarnings = FALSE)
