...
freyja_grouping <- s %>%
  ...
  mutate(lineages = gsub("\\([0-9]*\\)", "", lineages),
         lineages = ifelse(lineages == "Misc", "Unresolved", lineages),
...
freyja_grouping %>%
  ...
  mutate(relative_abundances = ifelse(is.na(relative_abundances), 1, relative_abundances),
         combined_lineage = ifelse(is.na(combined_lineage), "Unresolved", combined_lineage),
  ...
  mutate(combined_lineage = factor(combined_lineage,
                                   levels = c(str_sort(levels(combined_lineage)[!levels(combined_lineage) %in% other_lineage_fct_lvl], numeric = TRUE),
