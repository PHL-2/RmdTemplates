...
aws_s3_fastq_files_bucketdir <- system2("ssh", c("-tt", ec2_hostname,
                                                 shQuote(paste("aws s3 ls", nf_demux_bucket_path, "--recursive",
                                                               ...
                                                               "| grep -v 'Merged'"), type = "sh")), # wastewater specific: ignore merged fastq files
                                        stdout = TRUE, stderr = TRUE) %>%
  head(-1)
...
if(!is_nf_concat_samplesheet_empty) {
  ...
  upload_concat_session_path <- paste(tmp_screen_path, upload_concat_session, sep = "/")

  # Transfer sample sheet
  mk_remote_dir(ec2_hostname, upload_concat_session_path)
  run_in_terminal(paste("scp", local_concat_samplesheet_fp,
                        paste0(ec2_hostname, ":", upload_concat_session_path))
  )
  ...
  submit_screen_job(message2display = "Uploading samplesheet to S3",
                    ...
                    command2run = paste("aws s3 cp",
                                        paste0(upload_concat_session_path, "/", basename(local_concat_samplesheet_fp)),
                                        nf_concat_fastq_samplesheet_fp)
  )

  check_screen_job(message2display = "Checking samplesheet upload job",
                   screen_session_name = upload_concat_session)

  # Run the concat nextflow pipeline
  concat_session <- paste0("nf-concat-fastq-", session_suffix)
  nf_headnode_screen_job(batch_job_queue = nf_general_jq,
                         nf_bucket_dir = nf_concat_bucket_path,
                         message2display = "Concatenating FASTQ files",
                         screen_session_name = concat_session,
                         command2run = paste(ami_aws_cli, "s3 cp", paste(s3_aux_files_bucket, "external_scripts", "nextflow", "concat_fastq.nf", sep = "/"), ".;",
                                             "nextflow run concat_fastq.nf",
                                             "-profile", nextflow_profiles$base,
                                             "-bucket-dir", nf_concat_bucket_path,
                                             "-resume",
                                             "--use_container", nf_base_container,
                                             "--concat_samplesheet", nf_concat_fastq_samplesheet_fp,
  ...
  all_fastq_file_sizes <- read_csv(local_concat_samplesheet_fp) %>%
