...
```{r manual input ARTIC primer bed file to use, echo=FALSE}
...
s_meta <- read_delim(mapping_file_fp, delim = ",") %>%
  mutate(isControl = sample_type %in% ctrl_fct_lvl[-1]) %>%
...
```{r aws cli demux and cecret commands}

# tar.gz the run folder:
# tar -czvf [sequencing run name].tar.gz -C /path/to/[sequencing run folder]/ .
# (if you are taring from the parent directory of the sequencing runs, don't include a period in front of the sequencing run folder)
#
# upload the tar run folder to s3 bucket (there is no way to get the SHA256 of object upload with 'aws s3 cp' at the moment)
# 
# run the nf-core demux pipeline (running this with AWS Batch takes 33 minutes with 18 samples)
# 
# check that all the sample fastq files are available
#
# run the Cecret pipeline
# 
# download the results

# aws s3 cp s3://nextflow-output/COVIDSeq/[sequencing_date]_COVIDSeq/ . \
#   --recursive \
#   --exclude "*" \
...
non_ctrl_samples <- s_meta %>%
  filter(!sample_type %in% ctrl_fct_lvl) %>%
  select(sample_id) %>%
  pull()
...
if(!all(snpdist_check$snps == 0)) {
  stop("Something wrong with the snp-dist process")
}
...
```{r merge metadata with data and select samples with results, echo=FALSE}
...
lvls <- c("Reference", "Unassigned reads", "Water control", "Reagent control", "Environmental control", "Mock DNA positive control",
...
ann_geom_values[["sample_type_colors"]] <- setNames(c("#000000", pal_d3("category20")(20)[c(15, 19, 16, 13, 2, 1, 4, 10, 7, 8)]), lvls)
...
ann_geom_values[["sample_type_shapes"]] <- setNames(c(4, 8, 24, 13, 25, 23,
                                                      21, 22, 21, 22,
                                                    c(9:14)[1:(length(lvls)-10)]), lvls)

#make the controls and unassigned reads samples bigger in size
ann_geom_values[["sample_type_sizes"]] <- setNames(c(4, 4, 2, 4, 2, 4, rep(1, length(lvls)-6)), lvls)

#make alpha for nasal swabs to 0.5
ann_geom_values[["alpha"]] <- setNames(c(1, 1, 1, 1, 1, 1, rep(0.2, length(lvls)-6)), lvls)
