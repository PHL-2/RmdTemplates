...
gisaid <- s_toPlot %>%
  ...
  mutate(`Collection date` = case_when(!is.na(sample_collection_date) ~ sample_collection_date,
...
all_sample_ids_from_csv <- csvs_2_upload %>%
  ...
  filter(!grepl("accession_number|Virus name|sample_name|Library_ID", X1)) %>%
...
#can change this to 5 if you don't want the fastq files
