...
samp_sh_s <- data.frame(X1 = unlist(sample_sheet$Data), stringsAsFactors = FALSE) %>%
  ...
  #select(SampleName, I7_Index_ID, I5_Index_ID, Barcode_sequence, Sample_Type, Index_ID)
  rename(SampleName = "Sample_Name", Index_ID = "I7_Index_ID") %>%
  mutate(I7_Index_ID = index, I5_Index_ID = index2) %>%
  mutate(Barcode_sequence = paste0(I7_Index_ID, "+", I5_Index_ID)) %>%
  select(SampleName, I7_Index_ID, I5_Index_ID, Barcode_sequence, Index_ID) %>%
...
s <- s_meta %>%
  merge(samp_reads, by = c("sample_id", "Index_ID"), all = TRUE) %>%
