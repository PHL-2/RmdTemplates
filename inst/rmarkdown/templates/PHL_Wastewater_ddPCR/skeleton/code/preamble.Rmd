...
if (variance_test | signal_check) {
  ...
  wellsToRemove2 <- d_sd %>%
    filter(Target == "MHV",
           Positives < 100 | outlier_median >= 9| GESD_q <= 0.01, 
           grepl(pattern = "NorthEast|SouthEast|SouthWest", x = Type),
           Process == "Concentrated",
           Spiked == "Spiked",
           ) %>%
...
ww_nwss_metadata <- read_csv(ww_nwss_meta_fp) %>%
...
towrite <- ww_SC2_conc_result_summarised %>%
  ...
  left_join(wwMeta, by = c("sample_id" = "sample_group"))
...
sequencing_metadata <- d %>%
  ...
  mutate(stock_conc = stock_concentration_calc(concentration = avg_conc, 
                                               ...
                                               efficiency = 1, 
                                               end_concentration_vol = end_concentration_vol_uL,
                                               initial_concentration_vol = init_concentration_vol_uL)*1000*1000) %>%
  ungroup() %>%
  pivot_longer(cols = c(avg_conc, stock_conc, target_present), names_to = "concentration", values_to = "values") %>%
  mutate(Target = case_when(concentration == "avg_conc" ~ paste0(Target, "_gc/uL"),
                            ...
                            TRUE ~ NA)) %>%
  select(sample_group = "Type", ddpcr_replicates, sample_collect_date = "Sample_date", Target, values) %>%
...
write_csv(sequencing_metadata, here("sequencing_metadata",
                                    paste0(as.Date(gsub("_.*", "", project_name)), "_ww_sequencing_metadata.csv")))
