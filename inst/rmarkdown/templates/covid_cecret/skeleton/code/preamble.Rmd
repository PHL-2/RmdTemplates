...
unzip(here("data", "processed_cecret", "nextclade", "sars.zip"),
      exdir = here("data", "processed_cecret", "nextclade"), files = "tag.json")

nc_dl_fp <- here("data", "processed_cecret", "nextclade", "tag.json")

nc_dl_fp %>%
  read_csv(col_names = FALSE) %>%
  rename(version = "X1") %>%
  filter(grepl("tag", version)) %>%
  mutate(pipeline = "UPHL-BioNGS-Cecret",
         software = "nextclade-dataset",
         version = gsub("tag: |,", "", version)) %>%
  select(pipeline, software, version) %>%
  write_csv(file = software_version_fp, append = TRUE)
...
if(nf_artic_version != artic_primer_scheme) {
    stop(simpleError("The Nextflow ARTIC primer set does not match the selected primer set in this RStudio project. Investigate"))
}
...
if(have_phi) {
  ...
  epi_2_upload_fp <- list.files(here("upload", "epi"), pattern = "COVIDSeq_PHL_sequencing_results.csv", recursive = TRUE, full.names = TRUE)
...
slide_window_coverage %>%
  ...
  ggplot(., aes(x = position, y = slide_window_100, fill = sample_type, group = sample_id)) +
    ...
    labs(x="Genomic coordinates (bp)", y="Sliding window average (X)")
  } %>%
