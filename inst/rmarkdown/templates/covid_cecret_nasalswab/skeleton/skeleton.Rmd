...
ssh_seqsender_cmd <- function(pos_argument,
  ...
  if (pos_argument == "prep gisaid") {
    pos_argument <- "prep"
    additional_flags <- paste("-g", additional_flags)
    success_msg <- success_msg[grepl("GISAID", success_msg)]

  } else if (pos_argument == "prep ncbi") {
    pos_argument <- "prep"
    additional_flags <- paste("-bsn", additional_flags)
    success_msg <- success_msg[grepl("BIOSAMPLE|SRA|GENBANK", success_msg)]

  # in the current version of seqsender, the password field is only limited to ^[a-zA-Z0-9_.+-]+ characters
  # once this issue is resolved, can try submitting to gisaid through this program instead of covCLI
  # } else if (pos_argument == "submit --gisaid") {
  #   additional_flags <- paste(additional_flags, is_test)
  #   
  #   success_msg <- c(success_msg[grepl("GISAID", success_msg)],
  #                    "Connecting to NCBI FTP Server",
  #                    paste0("Submission name: ", submission_name, "_GISAID"),
