...
s <- s_meta %>%
  ...
  mutate(ww_group = case_when(grepl("^Undetermined$", sample_id) ~ "Unassigned reads",
         ...
         sample_group = factor(sample_group,
                               levels = unique(c("Unassigned reads",
                                                 ctrl_fct_lvls,
                                                 str_sort(unique(sample_group), numeric = TRUE)))),
...
```{r download pango names and CDC Tracker lineages}
...
cdc_tracker_url <- "https://data.cdc.gov/resource/jr58-6ysp.json?$select=variant"
