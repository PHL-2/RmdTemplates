...
if(sum(grepl(pattern = "Sample description", x = colnames(d_pre)))>0) {
  d_pre <- d_pre %>%
    ...
    mutate(percentage_positive = Positives/Droplet_count * 100,
          ...
          Sample_date = as.Date(parse_date_time(Sample_date, orders = c("%m/%d/%Y", "%m/%d/%y", "%Y-%m-%d"))),
...
LIMS <- d_sd %>%
  mutate(flag = ifelse((Type == "NorthEast" | Type == "SouthEast" | Type == "SouthWest") & (outlier_median > 9) & (Target == "N2" | Target == "E") | Droplet_count < 10000 | (Target == "MHV" & Droplet_count < 100), TRUE, FALSE),
         ...
         uniq_sample_name = paste0("WW-", Sample_date, "-", Type),
  ...
  right_join(d_pre) %>%
  select(uniq_sample_name,Type, Sample_date, `Conc(copies/uL)`, Droplet_count, Positives, Negatives, Pos10k, avg_conc, sd_conc, rxn_vol_uL, input_vol_uL, extraction_vol_uL, elution_vol_uL, end_concentration_vol_uL, init_concentration_vol_uL, outlier_median, flag, flagComment)
