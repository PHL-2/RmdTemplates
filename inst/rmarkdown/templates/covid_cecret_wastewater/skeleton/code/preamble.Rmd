...
if(nrow(seqsender) != 0) {
  ...
  seqsender <- seqsender %>%
    ...
    mutate(collection_date = case_when(!(is.na(sample_collect_date) |
           ...
           `bs23-collection_site_id` = wwtp_name,
           `bs24-ww_population` = population_served,
           #bs25-ww_surv_target_1
           #bs26-ww_surv_target_1_gene
           `bs27-ww_surv_target_1_known_present` = ifelse(has_N2,
                                                          ifelse(N2_present & !is.na(N2_present), "Yes", "No"),
                                                          ""),
           `bs28-ww_surv_target_1_conc` = ifelse(is.na(`N2_gc/L`), "", `N2_gc/L`),
           #bs29-ww_surv_target_1_conc_unit
           #bs30-ww_surv_target_1_protocol
           #bs31-ww_surv_target_2
           #bs32-ww_surv_target_2_gene
           `bs33-ww_surv_target_2_known_present` = ifelse(has_N2,
                                                          ifelse(E_present & !is.na(E_present), "Yes", "No"),
                                                          ""),
           `bs34-ww_surv_target_2_conc` = ifelse(is.na(`E2_gc/L`), "", `E2_gc/L`),
           #bs35-ww_surv_target_2_conc_unit
           #bs36-ww_surv_target_2_protocol
           `bs37-gisaid_virus_name` = `gs-sample_name`,
           #bs38-gisaid_accession
           #bs39-sample_title
  ...
  seqsender_fasta <- seqsender %>%
    filter(ww_group == "Wastewater sample - Merged")
  
  seqsender_fastq <- seqsender %>%
    filter(ww_group != "Wastewater sample - Merged")

  seqsender_something %>%
    write_csv(seqsender_meta_fp, na = "")
  #may have to split seqsender file into two files for fasta vs fastq submission. Not sure yet, try making the prep files from the same file
