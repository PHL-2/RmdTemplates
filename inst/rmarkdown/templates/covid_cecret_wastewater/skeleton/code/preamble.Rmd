...
freyja_grouping %>%
  ...
  select(ww_group, sample_name, sample_collect_date,
...
get_db_specific_fields <- function(df, db_name) {
  df %>%
    filter(db == db_name) %>%
    select(db_specific_field, value) %>%
    pivot_wider(names_from = "db_specific_field", values_from = "value")
}
...
data_upload_all %>%
  select(wwtp_name, sample_collect_date, zipcode, population_served, lod_sewage) %>%
  cbind(data_submit_metadata %>%
          get_db_specific_fields("NWSS")) %>%
  write_csv(nwss_upload_fp)

data_upload_all %>%
  select(covv_virus_name,
  ...
  mutate(fn = "???") %>%
  cbind(data_submit_metadata %>%
          get_db_specific_fields("GISAID")) %>%
  write_csv(gisaid_upload_fp)

data_upload_all %>%
  select(sample_name = "biosample_name",
         organism,
         ...
         design_description) %>%
  cbind(data_submit_metadata %>%
          get_db_specific_fields("BioSample")) %>%
  mutate(description = paste(sample_description, design_description),
        #need to fill these in
         ...
         gisaid_accession = "") %>%
  select(-c(sample_description, design_description)) %>%
...
data_upload_all %>%
  ...
  mutate(filename = "???",
         filename2 = "???") %>%
  cbind(data_submit_metadata %>%
          get_db_specific_fields("SRA")) %>%
  write_csv(sra_upload_fp)


#additional items to add
#is lod_sewage the same as ww_flow? Seems like flow data is separate
