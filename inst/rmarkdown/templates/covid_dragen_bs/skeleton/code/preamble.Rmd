...
```{r upload data to basespace and run the app, eval = dl_n_import_data}
...
shell_std_out <- system2(sh_exe_fp,                                             #the git shell executable on windows
                          args = c(                                              #some arguments need to be in quotes to be passed to shell
                            shQuote(bs_upload_shell_fp, type = "cmd"),           #the shell script to run
                            project_name,                                        #argument $1
                            fastq_folder,                                        #argument $2
...
cp_list <- c(list.files(here(run_folder, "Alignment_1"), pattern = "CompletedJobInfo.xml", recursive = TRUE, full.names = TRUE),
             list.files(here(run_folder, "Alignment_1"), pattern = "AdapterTrimming.txt", recursive = TRUE, full.names = TRUE),
...
#copy to
sapply(cp_list, file.copy, here("data", "demux_reads"))
...
sapply(bed_zip_files, function(x) unzip(x, exdir = dirname(x), files = paste0("consensus/", gsub(".*/|_.*", "", x), ".consensus_coverage_from_filtered_bam.bed")))
...
cov_files_fps <- list.files(here("data", "processed_bs"), pattern = "consensus_coverage_from_filtered_bam.bed", recursive = TRUE, full.names = TRUE)
...
coverage <- cov_files_fps %>%
  ...
  select(sample_id, X1, X4) %>%
  filter(X1 == "NC_045512.2") %>%
  group_by(sample_id) %>%
  summarise(pct_genome_coverage_over_30x = sum(X4 >= 30)/n(),
            median_coverage = median(X4)) %>%
  ungroup()
