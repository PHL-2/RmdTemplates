...
tryCatch(
  ...
  error = function(e) {
    print("If there's an error that pops up saying you need to accept the license, actually run the newest version of the DRAGEN COVID Lineage App once to accept license (you can abort it and return to using the shell script)")
  }
)
...
```{r load coverage and program versions, eval = dl_n_import_data}

cov_files_fps <- list.files(here("data", "processed_bs"), pattern = "_report_metrics.json", recursive = TRUE, full.names = TRUE)
...
coverage <- cov_files_fps %>%
  ...
  ungroup() %>%
  select(-c(FileName)) %>%
  mutate(X5 = ifelse(is.na(X5), X4, X5)) %>%
  mutate(X4 = ifelse(grepl("^callability_threshold: ", X4), X3, X4)) %>%
  mutate(X3 = ifelse(grepl("^result: ", X3), NA, X3)) %>%
  mutate(X9 = gsub("NC_045512.2: ", "", X9)) %>%
  mutate_all(~gsub('\\{|\\}|\\"', '', .)) %>%
  #rename columns with text by picking a row without any NA's and gsubbing the colon and everything after
  `names<-`(replace(names(.),
                    1:ncol(.),
                    (filter(., if_all(everything(), ~ !is.na(.x))) %>% slice(1) %>% gsub(":.*", "", .)))) %>%
  mutate_all(~gsub('.*: ', '', .)) %>%
  rename(pct_genome_coverage_over_30x = "pct_of_genome_with_coverage_30x_inf", sample_id = "sample", pango_pusher_data_version = "pangolin_version") %>%
  mutate(pct_genome_coverage_over_30x = as.numeric(pct_genome_coverage_over_30x)/100) %>%
  mutate(median_coverage = gsub(".0$", "X", median_coverage))
...
nextclade_version <- as.character(gsub(".* ", "", unique(bs_results[!is.na(bs_results$nextclade_version), "nextclade_version"])))
