...
read_type <- s %>%
  select(sample_id, sample_type, read_counts, samtools_reads_mapped) %>%
  ...
  select(sample_id, sample_type, read_counts, `total read counts`, mapped) %>%
...
```{r freyja results grouped, fig.height=11, fig.width=8}
...
freyja_grouping <- s %>%
  ...
  mutate(least_common_substring = attr(adist(lineages, counts = TRUE), "trafos")[, 1]) %>%
  mutate(end_match = do.call(rbind, stringi::stri_locate_all_regex(least_common_substring, "^M+"))[, 2]) %>%
  mutate(has_lowest_str_length = str_length(lineages) == min(str_length(lineages))) %>%
  mutate(substring_end = ifelse(sum(has_lowest_str_length) == 1 &
                                grepl("^M+$", least_common_substring), end_match, NA)) %>%
  mutate(substring_end = ifelse(all(is.na(substring_end)), min(end_match), substring_end)) %>%
  fill(substring_end, .direction = "down") %>%
  ungroup() %>%
  mutate(combined_lineage = stringi::stri_sub(lineages, 1, substring_end))

other_lineage_fct_lvl <- c("Other lineages", "No freyja results")

freyja_grouping %>%
  group_by(sample_id, sample_type, combined_lineage) %>%
  summarize(relative_abundances = sum(relative_abundances)) %>%
  mutate(combined_lineage = ifelse(is.na(combined_lineage), "No freyja results", combined_lineage)) %>%
  mutate(combined_lineage = gsub("\\.$", "", combined_lineage)) %>%
  mutate(relative_abundances = ifelse(combined_lineage == "No freyja results", 1, relative_abundances)) %>%
  mutate(combined_lineage = fct_lump_n(combined_lineage, n = 3, w = relative_abundances, other_level = "Other lineages")) %>%
  ungroup() %>%
  mutate(combined_lineage = factor(combined_lineage,
                                   levels = c(levels(combined_lineage)[!levels(combined_lineage) %in% other_lineage_fct_lvl],
  ...
  order_on_other_col(sample_id, sample_type, decreasing = FALSE) %>%
  {
  ggplot(., aes(x = sample_id, y = relative_abundances, fill = combined_lineage)) +
    geom_col() +
...
```{r freyja results summarized, fig.height=11, fig.width=8}
