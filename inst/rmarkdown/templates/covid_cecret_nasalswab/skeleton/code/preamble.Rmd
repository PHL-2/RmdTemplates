...
seqsender_config_fp <- list.files(file.path(dirname(here()), "aux_files", "data_submission", "seqsender"), pattern = "seqsender_config.yaml", full.names = TRUE)
seqsender_config_test_fp <- list.files(file.path(dirname(here()), "aux_files", "data_submission", "seqsender"), pattern = "seqsender_config_test.yaml", full.names = TRUE)
...
seqsender <- s_toPlot %>%
  ...
  rename(sequence_name = "sample_id",
         ...
         #BioSample fields
         `bs-host` = "host_scientific_name",
         `bs-host_disease` = "host_disease") %>%
  mutate(collection_date = case_when(!(is.na(sample_collection_date) |
         ...
         `bs-sample_name` = ifelse(vadr_passfail == "PASS",
                                   paste0("SARS-CoV-2/Human/USA/", sequence_name, "/", collection_year),
                                   ""),
         `bs-collected_by` = `gs-covv_orig_lab`,
         `bs-isolate` = `bs-sample_name`,
         `bs-isolation_source` = `gs-covv_specimen`,
         `bs-host_age` = `gs-covv_patient_age`,
         `bs-host_sex` = ifelse(`gs-covv_gender` == "Unknown", "Not provided", `gs-covv_gender`),
         `bs-gisaid_virus_name` = `gs-sample_name`,
         #SRA fields
         `sra-sample_name` = paste0("SARS-CoV-2/Human/USA/", sequence_name),
         `sra-file_1` = paste0(sequence_name, "_S", sample_number, "_filtered_R1.fastq.gz"),
         `sra-file_2` = paste0(sequence_name, "_S", sample_number, "_filtered_R2.fastq.gz"),
         `sra-library_name` = paste0("ARTIC Network Protocol ", artic_primer_scheme),
         `sra-instrument_model` = gsub("^Illumina NextSeq", "NextSeq", `gs-covv_seq_technology`),
         `sra-design_description` = paste0("Viral sequencing was performed following a tiling amplicon strategy using the ARTIC Network Protocol ", artic_primer_scheme,
                                           " primer scheme. Sequencing was performed using the ", `gs-covv_seq_technology`,
                                           " instrument with 2x", read_length,
                                           " bp chemistry. Libraries were prepared using the Illumina COVIDSeq Assay."),
         #GenBank fields
         `gb-sample_name` = sequence_name,
         `gb-src-isolate` = `bs-sample_name`,
         `gb-src-host` = `bs-host`,
         ...
         `gb-cmt-Sequencing Technology` = `sra-instrument_model`) %>%
  cbind(data2submit_metadata) %>%
  select(sequence_name, organism, collection_date,
         matches("^gs-", ignore.case = FALSE),
         matches("^bs-", ignore.case = FALSE),
         matches("^sra-", ignore.case = FALSE),
