...
if (variance_test) {
  print(paste("Detected high variation; wells", paste(wellsToRemove, collapse = ", "), "have been removed", collapse = ""))
}
...
```{r checking droplet count, eval = droplet_test, fig.width=10}
...
```{r variance check, eval = variance_test}

if (variance_test) {
  if (all_var_check) {
    print("Detected high variation; wells effected will not affect summarized results so no wells removed")
  } else {
    print(paste("Detected high variation; wells", paste(wellsToRemove, collapse = ", "), "have been removed", collapse = ""))
  }
...
<<<<<<< HEAD
    filter(MAD_from_med >=9 | GESD_q <= 0.01) %>%
=======
    filter(MAD_from_med >= 9 | GESD_q <= 0.01) %>%
>>>>>>> 7e981016ed5d27918d8d479cda64993c7451cdee
    arrange(Type, Target) %>%
    distinct()
