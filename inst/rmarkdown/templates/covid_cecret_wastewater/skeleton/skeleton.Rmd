...
```{r GISAID BioSample SRA submission, message=TRUE, include=FALSE, eval=FALSE}
  ...
  #add a check to make sure the uni sample ID (bs-isolate?) in the fastq seqsender are all found in the uni sample name in fasta seqsender
...
ssh_seqsender_cmd <- function(pos_argument,
  ...
  if (pos_argument == "prep --gisaid") {
    ...
    additional_flags <- paste(additional_flags, "--fasta_file", fasta, is_test)
...
fastq_seqsender %>%
  select(-c(`bs-gisaid_accession`)) %>%
  left_join(select(gisaid_added_seqsender, seqsender_merge_check_cols, `bs-gisaid_accession`), by = seqsender_merge_check_cols) %>%
