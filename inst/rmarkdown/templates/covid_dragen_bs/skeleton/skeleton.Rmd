...
```{r upload data if results look good, eval=FALSE, include=FALSE, message=FALSE}
...
if(!all(file_create_success_msg %in% upload_files_created)) {
  stop(simpleError("Seqsender failed in created some of the upload files!"))
}
...
if(test_upload) {
    ...
    #for the real submission, include the GISAID Accession number for the SRA and GenBank submissions
    ...
    if(gisaid_number_submitted != nrow(seqsender) & gisaid_number_failed != 0) {
      stop(simpleError("Number of GISAID samples uploaded do not match seqsender numbers or number of samples failed exceed 0."))
    }
    ...
    file.remove(c(list.files(here("biosample_sra"), full.names = TRUE),
                  list.files(here("genbank"), full.names = TRUE)))
