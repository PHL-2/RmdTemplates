...
Shannon diversity index shows the number of lineages weighted by abundance. Higher numbers mean greater "eveness" (e.g. one hugely abundant lineage and 9 scarce ones has a lower shannon value than 10 equally abundant lineages)
  ...
  { if(use_kable)
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      ...
      kable("latex", longtable = T, digits=2, booktabs=T, escape=F,
          col.names = linebreak(colnames(.), align = "l")) %>%
