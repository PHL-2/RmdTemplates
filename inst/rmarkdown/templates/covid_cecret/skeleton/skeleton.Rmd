...
if(test_upload) {
  ...
  gs_token_file <- file.path(aux_seqsender_fp, "gisaid_test.authtoken")
  gs_log <- here("gisaid", paste0(project_name, "_gisaid_test.log"))
  gs_failed_samples_fp <- here("gisaid", paste0(project_name, "_failed_gisaid_upload_test.csv"))
} else{
  test_flag <- ""
  config2use <- seqsender_config_fp
  gs_token_file <- file.path(aux_seqsender_fp, "gisaid.authtoken")
  gs_log <- here("gisaid", paste0(project_name, "_gisaid.log"))
  gs_failed_samples_fp <- here("gisaid", paste0(project_name, "_failed_gisaid_upload.csv"))
}
...
gisaid_upload_success_msg <- c(paste0("[", nrow(seqsender), " rows x 30 columns]"),
                               "failed_count: submissions failed: 0")
...
if(all(grepl("^EPI_ISL_0$", seqsender$gisaid_accession)) | all(is.na(seqsender$gisaid_accession))) {
  ...
  file.remove(gs_log, gs_failed_samples_fp)
  
  ### submit files to gisaid
  gisaid_uploaded <- cli_submit("cli3",
                                "upload",
                                c("--token", gs_token_file,
                                  ...
                                  "--log", gs_log,
                                  "--failed", gs_failed_samples_fp,
                                  "--proxy", Sys.getenv("https_proxy")),
                                shQuote_type = "cmd")
}

if(!all(gisaid_upload_success_msg %in% gisaid_uploaded)) {
  stop(simpleError(paste0("Something went wrong! GISAID upload failed. Have these samples been already uploaded??\n",
                          "There may have also been a proxy error, in which case, just restart R and rerun this chunk to resubmit")))
}

#include the GISAID Accession number for the SRA and GenBank submissions
read_gisaid_accessions <- read_csv(gs_log, col_names = FALSE) %>%
  select(X2) %>%
  mutate(X2 = gsub("msg: |}", "", X2))

gisaid_number_submitted <- read_gisaid_accessions %>%
  filter(grepl("submissions uploaded: ", X2)) %>%
  mutate(X2 = gsub("submissions uploaded: ", "", X2)) %>%
  pull() %>%
  as.numeric()

gisaid_number_failed <- read_gisaid_accessions %>%
  filter(grepl("submissions failed: ", X2)) %>%
  mutate(X2 = gsub("submissions failed: ", "", X2)) %>%
  pull() %>%
  as.numeric()
...
gisaid_accessions <- read_gisaid_accessions %>%
  filter(!grepl("submissions uploaded: |submissions failed: ", X2)) %>%
  separate(X2, into = c("gisaid_name", "gisaid_accession"), sep = "; ", extra = "merge") %>%
