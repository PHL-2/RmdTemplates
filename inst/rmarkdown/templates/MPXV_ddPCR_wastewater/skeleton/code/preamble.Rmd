...
standardCopyPath <- list.files(
  path = here(), 
  pattern = "Analyzed.csv", 
  full.names = TRUE) %>%
  file.exists

ampCopyPath <- list.files(
  path = here(),
  pattern = "_Amplitude.csv",
  full.names = TRUE,
  recursive = TRUE) 

useExportFolder <- !file.exists(ampyCopyPath, standardCopyPath)
...
if (file.exists(ampCopyPath)) { #check if amp data exists
...
} else if (file.exists(standardCopyPath)) { #if no amp data, look for analyzed csv file
...
```{r Reading ddPCR file and selecting useful columns, eval = file.exists(standardCopyPath)}
...
```{r Reading in amplitude data, eval = file.exists(ampCopyPath)}
