...
# Sequencing and sample quality control
...
## Coverage and N2 cp/uL of each sample

```{r coverage and N2, fig.width=10, fig.height=5, eval = has_N2}

s %>%
  filter(!is.na(`N2_gc/uL`)) %>%
  mutate(weekday_date = paste0(!!date_col, "_", wday(!!date_col, label = TRUE))) %>%
  droplevels() %>%
  {
  ggplot(., aes(x = `N2_gc/uL`, y = pct_genome_coverage_over_10x, fill = ww_group)) +
    ...
    facet_wrap(~weekday_date, nrow = 1) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(.$ww_group), names(ann_geom_values$sample_type_colors))]) +
    theme_bw() + 
...
```{r coverage results, fig.height=4, fig.width=10}
...
```{r freyja heatmap, fig.width=20, fig.height=10}
...
freyja_table_delim <- freyja_average %>%
  ...
  ungroup() %>%
  filter(avg_rel_abund >= 0.01 | (CDC_lineages == "A" & avg_rel_abund > 0)) %>%
...
freyja_average %>%
  ...
  group_by(sample_group, !!date_col) %>%
  mutate(has_A = any(CDC_lineages == "A"),
         is_control = grepl("Zepto|oldWW", sample_group)) %>%
  ungroup() %>%
    ...
    mutate_if(., function(x) is.character(x) | is.factor(x), ~gsub("%", "\\\\%", .)) %>%
      mutate(`Sample group` = cell_spec(`Sample group`, "latex",
                                        color = case_when((grepl("Merged", `Sample group`) &
                                                           !has_A &
                                                           !is_control &
                                                           !grepl("^B$|^Other lineages$", Lineage) &
                                                           good_cov) ~ "#4CBB17",
                                                          (!is_control &
                                                           grepl("^A$", Lineage) &
                                                           good_cov) ~ "#FF9896FF",
                                                          TRUE ~ "black"))) %>%
      select(-c(good_cov, has_A, is_control)) %>%
      select_all(~gsub(" ", "\n", .)) %>%
      select_all(~gsub("_", "\\\\_", .)) %>%
...
has_pos_ctrl <- s %>%
  ...
  mutate(is_pos = Nextclade_pango == "A",
         pos_ctrl_pct = sum(is_pos, na.rm = TRUE)/n()) %>%
  select(pos_ctrl_pct) %>%
  unique() %>%
  pull()
...
Number of samples to send to epidemiologists: `r reported_samples` \newline
Number of samples to upload to GISAID database: `r all_reported_samples` \newline
Number of samples to upload to BioSample: `r all_reported_samples` \newline
Number of samples to upload to SRA: `r all_reported_samples` \newline
  ...
  { if(use_kable) 
    ...
    mutate(SampleID = cell_spec(SampleID, "latex",
                               color = case_when((grepl(paste0(rerun_samples, collapse = "|"), sample_id) & length(rerun_samples) > 0) ~ "#FFD700",
                                                 ((grepl("control|Unassigned reads", `sample/site`)) | (!(sample_id %in% s_toReport$sample_id))) ~ "red",
                                                 TRUE ~ "black"))) %>%
      select(SampleID,
             `sample/site`,
             `read counts`,
...
```{r percent reads of lane, echo=FALSE, fig.height=7, fig.width=6}
...
The samples are processed on a 96-well plate. These figures illustrate the viral load, genome coverage, and lineage results of each sample in a well. Wells without any samples are white while samples with low values begin with dark blue.

### Viral load

```{r N2 on plate, fig.width=9, fig.height=12, eval = has_N2}
...
empty_plate %>%
  ...
  mutate(plate = paste("Plate", plate),
         ...
         sample_group = gsub(" ", "\n", sample_group),
         sample_label = ifelse(`N2_gc/uL` > mean(`N2_gc/uL`, na.rm = TRUE), "High", "Low")) %>%
  droplevels() %>%
  {
  ggplot(., aes(x=plate_col, y=fct_rev(plate_row), fill = `N2_gc/uL`, label = sample_group)) +
    geom_tile(fill = "white") +
    geom_point(shape = 21, size = 18) +
    geom_text(aes(color = sample_label), check_overlap = TRUE, size = 2) +
    scale_fill_viridis_c(na.value = "white") +
    facet_wrap(~plate, ncol = 1) +
    scale_color_manual(values = c(High = "#000000", Low = "#FFFFFF"), na.value = "#000000") +
...
```{r reads on plate, fig.width=9, fig.height=12}
...
empty_plate %>%
  ...
  mutate(plate = paste("Plate", plate),
         plate_col = as.numeric(plate_col),
         sample_group = gsub(" ", "\n", sample_group),
         sample_label = ifelse(read_counts > mean(read_counts, na.rm = TRUE), "High", "Low")) %>%
  droplevels() %>%
  {
  ggplot(., aes(x=plate_col, y=fct_rev(plate_row), fill = read_counts, label = sample_group)) +
    geom_tile(fill = "white") +
    geom_point(shape = 21, size = 18) +
    geom_text(aes(color = sample_label), check_overlap = TRUE, size = 2) +
    scale_fill_viridis_c(na.value = "white") +
...
```{r SC2 kmer numbers, fig.height=4, fig.width=10}
...
```{r SC2 kmer props, fig.height=8, fig.width=6}
...
```{r shannon diversity, fig.height=9, fig.width=9}
...
s %>%
  ...
  ggplot(., aes(x=Axis.1, y=Axis.2)) +
    geom_point(aes(fill = ww_group, shape = sample_group), size = 3) +
    scale_fill_manual(values = ann_geom_values$sample_type_colors[match(levels(s$ww_group), names(ann_geom_values$sample_type_colors))]) +
    scale_shape_manual(values = ann_geom_values[["sample_group_shapes"]]) + 
    #stat_ellipse(mapping = aes(color = sample_group)) +
    ...
    labs(x=paste0("PCoA axis 1 (", pct[1], "%)"), 
         ...
         color="Wastewater group", fill = "Wastewater group", shape="Control/Site",
         title="How similar are the samples?") +
    guides(fill = guide_legend(override.aes = list(shape = 21)))
  }
...
freyja_average %>%
  ...
  ggplot(., aes(x = isMerged, y = avg_rel_abund, fill = CDC_lineages)) +
    geom_col() +
    facet_wrap(as.formula(paste0("~", rlang::as_string(date_col), "+sample_group"))) +
