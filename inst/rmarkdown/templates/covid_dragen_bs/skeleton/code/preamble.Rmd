...
sequencing_folder_fp <- "C:/Users/vincent.tu/OneDrive - City of Philadelphia/RmdTemplates/test"
...
```{r upload data to basespace and run the app, message=TRUE, eval = dl_n_import_data}
# Generate the data folders

dir.create(here("data", "demux_reads"), recursive = TRUE)
dir.create(here("data", "processed_bs"), recursive = TRUE)

list_of_indexes <- list.files(run_folder, pattern = "*I[12]_001.fastq.gz", recursive = TRUE, full.names = TRUE)
...
bs_upload_shell_fp <- file.path(dirname(here()), "aux_files", "shell_scripts", "bs_cli_commands.sh")

fastq_folder <- dirname(list.files(run_folder, pattern = "R[12]_001.fastq.gz", recursive = TRUE, full.names = TRUE)[1])

#have to put the character vectors in quotes for them to run on shell
shell_std_out <- system2(sh_exe_fp,                                             #the git shell executable on windows
                          args = c(                                              #some arguments need to be in quotes to be passed to shell
                            shQuote(bs_upload_shell_fp, type = "sh"),           #the shell script to run
                            project_name,                                        #argument $1
                            shQuote(fastq_folder, type = "sh"),                  #argument $2
                            shQuote(app2run, type = "sh"),                       #argument $3
                            artic_primer_scheme,                                 #argument $4
                            shQuote(here("data", "processed_bs"), type = "cmd")  #argument $5
                         ), stdout = TRUE)

message(shell_std_out)
#If there's an error that pops up saying you need to accept the license, actually run the newest version of the DRAGEN COVID Lineage App once to accept license (you can abort it and return to using the shell script)


# YYYY-MM-DD
app_run_date <- as.character(as.Date(shell_std_out[length(shell_std_out)]))

# N.N.N
dragen_app_version <- shell_std_out[length(shell_std_out)-1]

#copy from
cp_list <- c(list.files(here(run_folder, "Alignment_1"), pattern = "CompletedJobInfo.xml", recursive = TRUE, full.names = TRUE),
             list.files(here(run_folder, "Alignment_1"), pattern = "DemultiplexSummaryF1L1.txt", recursive = TRUE, full.names = TRUE),
             list.files(here(run_folder, "Alignment_1"), pattern = "FastqSummaryF1L1.txt", recursive = TRUE, full.names = TRUE),
             list.files(here(run_folder, "Alignment_1"), pattern = "SampleSheetUsed.csv", recursive = TRUE, full.names = TRUE))

#copy to
sapply(cp_list, file.copy, here("data", "demux_reads"), TRUE)
...
```{r get nextclade version by downloading Undetermined file, eval = dl_n_import_data}
...
```{r unzip fastq files and bed files, eval = dl_n_import_data}
...
sapply(scrubbed_zip_files, function(x) unzip(x, exdir = dirname(x)))

#extract just the bed files
bed_zip_files <- zip_fps[grepl("_all_output_files", zip_fps)]
sapply(bed_zip_files, function(x) unzip(x, exdir = dirname(x), files = paste0("consensus/", gsub(".*/|_all_output_files.zip$", "", x), ".consensus_coverage_from_filtered_bam.tsv")))
#sometimes these files are bed and other times they are tsv files...
sapply(bed_zip_files, function(x) unzip(x, exdir = dirname(x), files = paste0("consensus/", gsub(".*/|_all_output_files.zip$", "", x), ".consensus_coverage_from_filtered_bam.bed")))


```

```{r load coverage results, eval = dl_n_import_data}

#this bed file can only be found in the zipped output files
cov_files_fps <- list.files(here("data", "processed_bs"), pattern = "consensus_coverage_from_filtered_bam", recursive = TRUE, full.names = TRUE)
...
```{r load kmer results, eval = dl_n_import_data}
...
```{r load Pangolin results, eval = dl_n_import_data}
...
```{r load NextClade results, eval = dl_n_import_data}
...
```{r write bs_results, eval = dl_n_import_data}

merge(coverage, kmer, by = "sample_id", all = TRUE) %>%
  merge(pango, by = "sample_id", all = TRUE) %>%
  merge(next_clade, by = "sample_id", all = TRUE) %>%
  mutate(date_pipeline_was_run = app_run_date) %>%
  mutate(dragen_covid_lineage_version = dragen_app_version) %>%
...
sample_type_fct <- c("Unassigned reads", "Empty well control", "Mock DNA positive control")
...
ann_geom_values[["sample_type_colors"]] <- setNames(pal_d3("category20")(20)[c(5, (1:length(lvls))[-5])], lvls)

#make the controls and unassigned reads into triangle and diamond shapes
ann_geom_values[["sample_type_shapes"]] <- setNames(c(23:25,
                                                      21:22,
                                                      c(26:30)[1:(length(lvls)-5)]), lvls)

#make the controls and unassigned reads samples bigger in size
ann_geom_values[["sample_type_sizes"]] <- setNames(c(4, 4, 4, rep(1, length(lvls)-3)), lvls)
...
for(variant in sort(unique(s$scorpio_call)[!is.na(unique(s$scorpio_call))])) {
  ...
  variant_list <- s %>%
    ...
    select(note, Nextclade_pango) %>%
    mutate(note = gsub("^.*: |; scorpio.*", "", note)) %>%
    separate(note, into = c("pusher_variant1", "pusher_variant2"), sep = " ", extra = "merge") %>%
    pivot_longer(cols = c("pusher_variant1", "pusher_variant2", "Nextclade_pango"), names_to = "variant_num", values_to = "variants") %>%
...
```{r save fastq for upload, eval = dl_n_import_data}
...
sapply(fastq2upload, function(x) file.rename(x, to = paste0(here("upload", "fastq"), gsub(".*scrubbed\\.[0-9]*\\.", "/", x))))
