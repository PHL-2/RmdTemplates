...
s %>%
  ggplot(aes(x = library_conc_ng_ul, y = read_counts, fill = sample_type)) +
...
## Library concentration, read counts, `r covid_quant_unit` values per plate
...
empty_plate %>%
  ...
  ggplot(aes(x=plate_col, y=fct_rev(plate_row), fill = ct)) +
    ...
    {
      if(all(is.na(s$library_conc_ng_ul))) scale_fill_viridis_d(na.value = "white")
    ...
    labs(
      x="",
      y="Plate row",
      fill=paste(covid_quant_unit, "values")
    )
...
empty_plate %>%
  ...
  droplevels() %>%
  mutate(plate = paste("Plate", plate)) %>%
  ggplot(aes(x=plate_col, y=fct_rev(plate_row), fill = library_conc_ng_ul)) +
    geom_tile(fill = "white") +
    geom_point(shape = 21, size = 10) +
    facet_wrap(~plate, ncol = 1) +
    #if there is no library concentration, use the discrete fill
    {
      if(all(is.na(s$library_conc_ng_ul))) scale_fill_viridis_d(na.value = "white")
      else scale_fill_viridis_c(na.value = "white")
    } +
...
rmarkdown::render(qc_report_fp, output_format = rmarkdown::md_document(variant = "markdown_github",
                                                                       toc = TRUE), output_file = "README.md")
