...
The library was sequenced on the Illumina `r unique(na.omit(s$instrument_type))` platform at 2x`r read_length` bp reads on `r sequencing_date` \newline
...
Cluster density: `r unique(na.omit(s$run_cd))` \newline
Q30 score or higher: `r unique(na.omit(s$run_q30))*100`% \newline
Percent of reads that passed filter: `r unique(na.omit(s$run_pf))*100`% \newline
Run error rate `r unique(na.omit(s$run_error))*100`%
...
undetermined_read_numbers <- s %>%
  select(sample_id, read_counts) %>%
  mutate(total_reads = sum(read_counts, na.rm = TRUE),
         undetermined_percent_reads = read_counts/total_reads,
         more_than_half = undetermined_percent_reads > 0.5) %>%
  filter(sample_id == "Undetermined")

s %>%
  { if(undetermined_read_numbers$more_than_half) filter(., sample_id != "Undetermined")
    else .
  } %>%
  ggplot(aes(y=read_counts, x=sample_type, fill = sample_type)) +
    ...
    labs(
      title = paste0("Total number of reads: ",
                     round(undetermined_read_numbers$total_reads/1000000, digits = 2), "M",
                     ifelse(undetermined_read_numbers$more_than_half,
                            paste0(" (Undetermined: ", round(undetermined_read_numbers$read_counts/1000000, digits = 2), "M", ")"),
                            "")),
      y="Read counts",
      x="",
      fill = "Sample Type",
...
s %>%
  mutate(percent_of_lane = read_counts/sum(read_counts, na.rm = TRUE)) %>%
  group_by(sample_type) %>%
  mutate(tot_percent = sum(percent_of_lane, na.rm = TRUE)) %>%
  ungroup() %>%
...
actual_read_lengths %>%
  ...
  ggplot(aes(y=count, x=read_length, fill = sample_type)) +
    ...
    scale_y_continuous(labels = number_format(accuracy = 1)) +
    scale_x_reverse() +
    facet_grid(sample_type~process, scales = "free_y", 
    ...
    labs(
      y="Read counts",
      x="Read length",
      fill = "Sample Type", size = "Sample Type"
    )
...
```{r CT and DNA, fig.width=10, fig.height=5, eval = (has_CT & has_qubit)}
...
```{r RLU and DNA, fig.width=10, fig.height=5, eval = (has_RLU & has_qubit)}
...
The samples are processed on a 96-well plate. These figures illustrate the viral load, DNA concentration, and read counts of each sample on a well. Wells without any samples are white while samples with low values begin with dark blue. 
...
```{r variant assignments, fig.height=9, fig.width=7, eval = (nrow(s_toPlot) > 0)}
...
```{r multiple variant assignments, fig.height=9, fig.width=8, eval = (nrow(s_toPlot) > 0)}
...
```{r conflicting assignments, fig.height=9, fig.width=8, eval = (nrow(s_toPlot) > 0)}
...
if(nrow(s_toPlot) > 0) {
  report_gisaid <- nrow(all_sample_ids_from_seqsender[all_sample_ids_from_seqsender$file == "gisaid_name", "sample_id"])
  report_sra <- nrow(all_sample_ids_from_seqsender[all_sample_ids_from_seqsender$file == "sra_name", "sample_id"])
  report_genbank <- nrow(all_sample_ids_from_seqsender[all_sample_ids_from_seqsender$file == "genbank_name", "sample_id"])
} else {
  n_samples_report <- report_gisaid <- report_sra <- report_genbank <- 0
}
...
Number of samples to upload to GISAID database: `r report_gisaid` \newline
Number of samples to upload to SRA: `r report_sra` \newline
Number of samples to upload to Genbank: `r report_gisaid`\newline
  ...
  { if(use_kable) 
    ...
    mutate(sample = cell_spec(sample, "latex",
                               color = case_when((grepl(paste0(rerun_samples, collapse = "|"), sample_id) & length(rerun_samples) > 0) ~ "yellow",
                                                 ((grepl("control|Unassigned reads", `sample type`)) | (!(sample_id %in% s_toPlot$sample_id))) ~ "red",
...
* Nextflow: `r software_version$Nextflow`
  ...
  + Cecret pipeline: `r software_version[["Cecret"]]`
    ...
    - Pangolin: `r software_version[["pangolin"]]`
      - Pangolin dataset: `r software_version[["pangolin-data"]]`
...
freyja_sample_type %>%
  ...
  ggplot(., aes(x = sample_type, y = avg_rel_abund, fill = combined_lineage)) +
    ...
    scale_fill_manual(values = 
                        c(colorRampPalette(brewer.pal(11, "Spectral"))(max(length(unique(.$combined_lineage))-2, 1)),
                          "#808080", "#000000")) +
