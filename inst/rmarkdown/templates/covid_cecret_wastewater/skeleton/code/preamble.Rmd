...
```{r download pango names and CDC Tracker lineages, eval = import_data}
...
cdc_lineages <- read_csv(cdc_tracker_fp) %>%
  select(variant) %>%
  unique()
...
freyja_grouping <- s %>%
  ...
  mutate(lineages = gsub("$", ".", lineages),
         longest_common_substring = sapply(lineages,
                                           function(x) max(sapply(gsub("$", ".", cdc_lineages$variant),
         ...
         isCDClineage = grepl(paste0("^", paste0(gsub("\\.", "\\\\.", cdc_lineages$variant), collapse = "\\.|^"), "\\."), lineages),
         lineages = gsub("\\.$", "", lineages),
         CDC_lineages = fct_other(gsub("\\.$", "", longest_common_substring),
                                  keep = cdc_lineages$variant,
                                  other_level = "Other lineages"),
         CDC_lineages = factor(CDC_lineages, levels = c(str_sort(cdc_lineages$variant, numeric = TRUE), "Other lineages"))) %>%
  droplevels()
...
freyja_average <- freyja_grouping %>%
  ...
  complete(nesting(sample_id, sample_group, isMerged), nesting(lineages, main_lineage), fill = list(relative_abundances = 0)) %>%
  group_by(sample_id, sample_group, isMerged, lineages, CDC_lineages, main_lineage) %>%
  summarize(relative_abundances = sum(relative_abundances)) %>%
  ungroup() %>%
  filter(!is.na(relative_abundances)) %>%
  group_by(sample_group, isMerged) %>%
  ...
  group_by(sample_group, isMerged, lineages, CDC_lineages, main_lineage) %>%
