...
s_toPlot <- s %>%
  filter(!grepl("None|Undetermined", sample_id),
         ...
         grepl("Wastewater sample - Merged|Mock DNA positive control", ww_group)) %>%
...
#filter s_toPlot to remove samples that have A lineage present
samples_with_A <- freyja_grouping %>%
  filter(grepl("^Wastewater sample", ww_group),
         CDC_lineages == "A") %>%
  select(sample_id) %>%
  unique() %>%
  pull()
...
s_toPlot <- s_toPlot %>%
  filter(!sample_id %in% samples_with_A)
...
seqsender <- s %>%
  filter(uniq_sample_name %in% s_toPlot$uniq_sample_name,
         !isControl)
