  ...
  freyja_grouping <- freyja_lineages %>%
    mutate(lineages = gsub("$", ".", lineages),
           longest_common_substring = sapply(lineages,
                                             function(x) max(sapply(gsub("$", ".", c("A", "B", cdc_lineages$variant)),
           ...
           CDC_lineages = fct_other(gsub("\\.$", "", longest_common_substring),
                                    keep = c("A", "B", cdc_lineages$variant),
                                    other_level = "Other lineages"),
           CDC_lineages = factor(CDC_lineages, levels = c(str_sort(cdc_lineages$variant, numeric = TRUE), "Other lineages", "A", "B"))) %>%
    droplevels()
...
freyja_grouping %>%
  filter(sample_id %in% s_toPlot$sample_id,
         !isControl,
         CDC_lineages %in% cdc_lineages$variant) %>%
